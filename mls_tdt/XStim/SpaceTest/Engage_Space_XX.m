...
%Engage_Space
% Lots of small cahnges trying to get rid of PD1status problem
...
if(exist1('H.spacefig') & get(H.recorddata,'Value'))
    update_dataFN;
end
...
% load 1st stimuli
fname = ['stimbuf_left_' num2str(hrtfdirmat(1,locind(randseq(1)))) ...
        '_' num2str(hrtfdirmat(2,locind(randseq(1))))];
evalstr = ['S232(''pushdisk16''' ',' [' ''' FN.temp_stim_path fname ''' '] ');'];
eval(evalstr);
evalstr1 = ['S232(''qpop16'''  ',BUF.L1);'];
eval(evalstr1);
fname = ['stimbuf_right_' num2str(hrtfdirmat(1,locind(randseq(1)))) ...
        '_' num2str(hrtfdirmat(2,locind(randseq(1))))];
evalstr = ['S232(''pushdisk16''' ',' [' ''' FN.temp_stim_path fname ''' '] ');'];
eval(evalstr);
evalstr2 = ['S232(''qpop16'''  ',BUF.R1);'];
eval(evalstr2);
...
S232('seqplay',BUF.playspec1);
       ...
       % load NEXT stimuli
        ...
        eval(['S232(''pushdisk16''' ',' [' ''' FN.temp_stim_path fname ''' '] ');']);
        evalstr1 = ['S232(''qpop16'''  ',BUF.L1);'];
        ...
        if trialnum == numtrials
            ...
            fname = ['stimbuf_right_' num2str(hrtfdirmat(1,locind(randseq(trialnum+1)))) ...
                '_' num2str(hrtfdirmat(2,locind(randseq(trialnum+1))))];
        end
        eval(['S232(''pushdisk16''' ',' [' ''' FN.temp_stim_path fname ''' '] ');']);
        evalstr2 = ['S232(''qpop16'''  ',BUF.R1);'];
        pause(DUR/1000 + .1)
    ...
    if(exist1('H.spacefig') & ~exist1('H.finalspikeratefig'))
