...
if(plotflag)
   ...
   %FD
   if(fdtest(1) ~= -1)
      hfd = subplot('Position',[0.05 horiz_pos(4) 0.15 subplot_ht(4)]);
      set(hfd,'FontSize',8);
      herrbar = errorbar(Neuron.fd_freqaxis{1}/1000,Neuron.fd_mean{1},...
         Neuron.fd_std{1}/Neuron.fd_testpars{1}.numreps);
      set(herrbar(2),'LineWidth',2);
      set(herrbar(1),'Color','r');
      axis square
      xlim([1 11]);
      xlabel('Frequency Dropped (kHz)');
      ylabel('Spikes/stim');
