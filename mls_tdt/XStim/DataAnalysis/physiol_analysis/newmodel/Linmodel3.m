%Linmodel3 - script descibing and testing a linear model of spectral integration in ILDAlone RS
clear;
Model = 'Linmodel3';
...
for cell_num = 19:20
   ...
   IA_piece1 = Neuron{cell_num}.ia_meansurf{2};
   IA_piece2 = Neuron{cell_num}.ia_meansurf{4};
   
   %minimize the model error
   Options = optimset('Display','iter','MaxIter',500,'TolFun',0.002,'MaxFunEvals',1000);
   V = [10 10]; %ILD tuning widths for model
   [V,fval,exitflag,output] = fminsearch('errorfun_spectralintegration7',...
      V,Options,IA_fullspec,IA_piece1,IA_piece2,ILD_piece1,ILD_piece2,IA_bestloc);
...
%Interpolate the missing values in the ILDAlone measurement
   ...
   title(str2mat([Model ' Predicted ILDAlone'],['r^{2} = ', num2str(cc(1,count))],['ILD widths = (', ...
         num2str(vars(count,1)), ' ', num2str(vars(count,2)), ')']),'FontSize',8);
   hold on;
   plot(space_contour(1,:),space_contour(2,:),'black','LineWidth',1.5);
   hold off;
