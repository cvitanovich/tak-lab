...
for cell_num = begin_cells:end_cells
   ...
   clear IA_linpredict ILD_bands freq_bands cc_dist_ild cc_actfreq cc_diff
   bandwidth = 1/2; %units of octaves
   ...
   for freq_num = 1:length(ILDf_freqaxis)
      ...
      [ActILD_temp(freq_num,:),ActILD_residual{freq_num}] = ...
         ...
         IA_meansurf(ILD_bands_sort_index(freq_num,:)),alpha_ILD,mode);
            
      %Calculate correlation between Activity & ILD_distance
      temp = corrcoef(dist_ild_sort(freq_num,:),IA_meansurf(di_sort_index(freq_num,:)));
      cc_actfreq(freq_num) = -1*temp(1,2);
   end
   
   cc_dist_ild = corrcoef(dist_ild');
   cc_maxind = find(cc_actfreq == max(cc_actfreq));
   cc_diff = (cc_actfreq/max(cc_actfreq)) - (cc_dist_ild(cc_maxind,:));
   cc_diff(cc_maxind) = 1;
   ...
   for freq_num = 1:length(ILDf_freqaxis)
      ...
      if(tonal_weights)
         %freq_weights(freq_num) = 1;
         freq_weights(freq_num) = max(Neuron{cell_num}.tif_meansurf(freq_num,:));
      else
         cc_threshhold = 0;
         if(cc_diff(freq_num) >= cc_threshhold) 
            freq_weights(freq_num) = cc_diff(freq_num);
         else
      ...
      [Actilddist(freq_num,:),result_residual{freq_num}] = locregress1(dist_ild_signed_sort(freq_num,:),...
         IA_meansurf(dis_sort_index(freq_num,:)),alpha_dist,mode);
      
      [junk,reverse_sort_index(freq_num,:)] = sort(dis_sort_index(freq_num,:));
      clear junk
      ...
      if(plot_dots)
         ...
         plot(ILD_bands_sort(freq_num,:),...
            IA_meansurf(ILD_bands_sort_index(freq_num,:)),'k.',...
         ...
         plot(dist_ild_signed_sort(freq_num,:),...
            IA_meansurf(dis_sort_index(freq_num,:)),'k.',...
            'MarkerSize',Dot_markersize);
         hold on
         plot(dist_ild_signed_sort(freq_num,:),...
   ...
   [IA_linpredict_azi,IA_linpredict_ele,temp] = ...
      array2diamond(IA_linpredict,IA_locs');
%Interpolate the missing values in the ILDAlone measurement
   ...
   temp = corrcoef(IA_linpredict,IA_meansurf);
   cc(1,cell_num) = temp(1,2)^2;
   
   %Find the correlation coefficient between the Tonal ILD/freq predicted and measured ILDAlone RS
   for cc_loc = 1:length(IA_locs)
      cc_index(cc_loc) = max(find(HRTFinfo.location_matrix(1,:) == IA_locs(cc_loc,1)...
         & HRTFinfo.location_matrix(2,:) == IA_locs(cc_loc,2)));
   end
   if(any(strcmp(fieldnames(Neuron{cell_num}),'tia_meanarray')));
      temp = corrcoef(IA_meansurf,Neuron{cell_num}.tia_meanarray(cc_index));
   ...
   set(gca,'FontSize',8,'Color',bkgndcolor,'FontName','Garamond')
   axis square
   plotdiam(IA_azi,IA_ele,IA_linpredict_diam);
      h = colorbar; set(h,'FontSize',8,'FontName','Garamond');
   colormap(cmap);
   ...
   set(gca,'FontSize',8,'FontName','Garamond')
   plot(IA_meansurf/max(IA_meansurf),...
