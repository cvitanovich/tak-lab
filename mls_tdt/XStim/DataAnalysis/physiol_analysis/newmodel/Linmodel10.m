...
for cell_num = 19:20
   ...
   Options = optimset('Display','iter','MaxIter',12000,'TolFun',0.002,'TolX',0.01,'MaxFunEvals',24000);
   V = [10*ones(1,size(freq_bands,1)) ...
      zeros(1,size(freq_bands,1))]; %ILD tuning widths & ILD peak shifts for model
   [V,fval,exitflag,output] = fminsearch('errfun_specint2',...
      V,...
      Options,...
   ...
   [err,result,V] = errfun_specint2(V,...
      Neuron{cell_num}.ia_meansurf{1},...
      new_if_surface,...
      new_if_freqaxis,...
      Neuron{cell_num}.tif_ildaxis,...
