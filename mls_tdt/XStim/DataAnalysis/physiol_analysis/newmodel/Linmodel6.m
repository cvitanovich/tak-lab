...
for num_loc = 1:size(ILD_matrix,2)
   ILD_matrix(:,num_loc) = fsmooth2(ILD_matrix(:,num_loc),3,length(ILD_matrix(:,num_loc)));
end
...
for cell_num = 19:20
   ...
   V = [ones(1,8) zeros(1,8) 10*ones(1,8)]; %Frequency weights, ILDopt_shifts, ILD tuning widths
   [V,fval,exitflag,output] = fminsearch('errorfun_spectralintegration12',...
      V,...
      Options,...
      IA_fullspec,...
      ILD_piece1,...
