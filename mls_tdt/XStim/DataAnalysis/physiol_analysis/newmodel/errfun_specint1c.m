...
%Function includes attempt to use information regarding how dependence upon distance from
%ILDoptimum changes with frequency band
...
%Get the frequency weights, using distance information (distance1.m)
...
for band_num = 1:size(freq_bands,1)
   ...
   ILD_opt = repmat(trunc_ILD_matrix(minfreqindex:maxfreqindex,IA_bestloc_ind),...
      1,size(trunc_ILD_matrix,2));
   dist(band_num,:) = (sqrt(mean((trunc_ILD_matrix(minfreqindex:maxfreqindex,:) -...
      ILD_opt).^2,1)).^V(lenV));
   tempcc = corrcoef(dist(band_num,:),IA_fullspec);
   fcc(band_num) = tempcc(1,2)^2;
end
ccind = find(abs(fcc) < 0.1);
fcc(ccind) = 0;
freq_weight = abs(fcc);
