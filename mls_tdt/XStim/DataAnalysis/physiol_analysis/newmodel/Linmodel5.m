...
for cell_num = 19:20
   ...
   %Get the HRTF and ILD/Freq frequency indices
   ...
   [y,ind_minfreq_1tif] = min(abs(Neuron{cell_num}.tif_freqaxis - min_freq));
   [y,ind_maxfreq_1tif] = min(abs(Neuron{cell_num}.tif_freqaxis - max_freq));
   %ILD_piece2
   ...
   [y,ind_minfreq_2tif] = min(abs(Neuron{cell_num}.tif_freqaxis - min_freq));
   [y,ind_maxfreq_2tif] = min(abs(Neuron{cell_num}.tif_freqaxis - max_freq));
   %ILD_piece3
   ...
   [y,ind_minfreq_3tif] = min(abs(Neuron{cell_num}.tif_freqaxis - min_freq));
   [y,ind_maxfreq_3tif] = min(abs(Neuron{cell_num}.tif_freqaxis - max_freq));
   %ILD_piece4
   ...
   [y,ind_minfreq_4tif] = min(abs(Neuron{cell_num}.tif_freqaxis - min_freq));
   [y,ind_maxfreq_4tif] = min(abs(Neuron{cell_num}.tif_freqaxis - max_freq));
   %ILD_piece5
   ...
   [y,ind_minfreq_5tif] = min(abs(Neuron{cell_num}.tif_freqaxis - min_freq));
   [y,ind_maxfreq_5tif] = min(abs(Neuron{cell_num}.tif_freqaxis - max_freq));
   %ILD_piece6
   ...
   [y,ind_minfreq_6tif] = min(abs(Neuron{cell_num}.tif_freqaxis - min_freq));
   [y,ind_maxfreq_6tif] = min(abs(Neuron{cell_num}.tif_freqaxis - max_freq));
   %ILD_piece7
   ...
   [y,ind_minfreq_7tif] = min(abs(Neuron{cell_num}.tif_freqaxis - min_freq));
   [y,ind_maxfreq_7tif] = min(abs(Neuron{cell_num}.tif_freqaxis - max_freq));
   %ILD_piece8
   ...
   [y,ind_minfreq_8tif] = min(abs(Neuron{cell_num}.tif_freqaxis - min_freq));
   [y,ind_maxfreq_8tif] = min(abs(Neuron{cell_num}.tif_freqaxis - max_freq));
   
   %Get ridge of ILD/Freq RS
   ILDf_ridge = max(Neuron{cell_num}.tif_meansurf,[],2);
   
   %Get weights for frequency bands
   for piece = 1:8
      weights(piece) = eval(['mean(ILDf_ridge(ind_minfreq_'...
            num2str(piece) 'tif:ind_maxfreq_' num2str(piece) 'tif))']);
   end
   ...
   Options = optimset('Display','iter','MaxIter',500,'TolFun',0.002,'TolX',0.1,'MaxFunEvals',1000);
   V = [10*ones(1,8)]; %ILD tuning widths for model
   [V,fval,exitflag,output] = fminsearch('errorfun_spectralintegration11',...
      V,...
      Options,...
      IA_fullspec,...
      ILD_piece1,...
...
%Interpolate the missing values in the ILDAlone measurement
   ...
   vars(count,1:8) = round(vars(count,1:8));
   title(str2mat('Model Predicted ILDAlone',['r^{2} = ', num2str(cc(1,count))],['ILDw = (', ...
         num2str(vars(count,1)), ' ',...
         num2str(vars(count,2)), ' ',...
         num2str(vars(count,3)), ' ',...
         num2str(vars(count,4)), ' ',...
         num2str(vars(count,5)), ' ',...
         num2str(vars(count,6)), ' ',...
         num2str(vars(count,7)), ' ',...
         num2str(vars(count,8)), ')',...
      ]),'FontSize',8);
   hold on;
   plot(space_contour(1,:),space_contour(2,:),'black','LineWidth',1.5);
   hold off;
