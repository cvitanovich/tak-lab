...
for cell_num = 1:1
   if(any(strcmp(fieldnames(Neuron{cell_num}),'tif_meansurf')));
      ...
      %Limit frequencies of HRTF data
      minfreq = min(Neuron{cell_num}.tif_freqaxis);
      maxfreq = max(Neuron{cell_num}.tif_freqaxis);      
      [y,hrtf_minfreqind] = min(abs(HRTFinfo.hrtf_freqs - minfreq));
      [y,hrtf_maxfreqind] = min(abs(HRTFinfo.hrtf_freqs - maxfreq));      
      ILD_matrix_freqs = ILD_matrix(hrtf_minfreqind:hrtf_maxfreqind,:);
      hrtf_freqaxis = HRTFinfo.hrtf_freqs(hrtf_minfreqind:hrtf_maxfreqind);
      ...
      ILD_matrix_focus = ILD_matrix_freqs(:,ILDmat_index);
      
      
      %Interpolate the Tonal meansurf for full frequency information
      disp('Interpolating tif_meansurf...')
      tif_freqaxis = minfreq:100:maxfreq;
      tif_ildaxis = minILD:0.5:maxILD;
      tif_meansurf = interp2(Neuron{cell_num}.tif_freqaxis,...
      ...
      V(cell_num,:) = 10*ones(1,length(Neuron{cell_num}.tif_freqaxis));
      [V(cell_num,:),err] = fminsearch('ilddist_weights',V(cell_num,:),Options,...
         New_ILDf_mat,...
         ILD_matrix_focus,...
         hrtf_freqaxis,...
         tif_meansurf,...
      ...
      lineint_V(cell_num,:) = V(cell_num,:);
      save d:\mlspezio\matlab\save\lineint_V lineint_V 
      [err,IA_tonalpredict] = ilddist_weights(V(cell_num,:),...
         New_ILDf_mat,...
         ILD_matrix_focus,...
         hrtf_freqaxis,...
         tif_meansurf,...
      ...
      hold on
      plot(hrtf_freqaxis,ILD_matrix_focus(:,IA_bestloc_ind(cell_num)),'w','LineWidth',2);
