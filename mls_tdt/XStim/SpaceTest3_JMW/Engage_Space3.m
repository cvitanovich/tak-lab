...
% ALTERRED to play Free Field sounds (currently with Fs = 40.1 kHz)
% KLUGE
TDT.Fs = 11025;
TDT.Fs = 22050;

temp_HRTF = FN.space_eq;
FN.space_eq = 'onesX.imp';
...
clear test_type
XStimParams.curr_ITD = round(str2num(get(H.ITD,'String')));
...
% scale
ABAval = 0.5*(mom(sound_left,2) + mom(sound_right,2));
scalefact = TDT.scalevalue/ABAval;
sound_left = round(scalefact*sound_left);
sound_right = round(scalefact*sound_right);
