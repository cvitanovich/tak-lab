...
% uses *.eq HRTF files
% VERSION to run two REGIMES
% NOTE: non-randomized SPLs, just play all SPLs before repeating, the order
% may be random and different for each regime and each rep
% changed Oct 12, 2009 so each regime and each rep is the same
% ALSO: saves basic noi file to disk for later possible calc of STRF (need
% to adjust with SPLs from param4
%*******************************************************************************
S232('trash');
S232('dropall');
...
nRegimemsecs = XStimParams.regime_duration;         % duration of REGIMES1 & 2

%%%%%%%%%%%%
McTestType = XStimParams.McTestType;
switch McTestType
    case 'SPLtest  '     % focalSPLs gives dB-10 for 1st and last focalSPL
        % SPLs for low prob and for hi prob REGIME 1
        ...
        % SPLs for low prob and for hi prob REGIME 2
        SPLs_hi2 = 10+(XStimParams.focalSPL2(1): XStimParams.SPLinc(2): XStimParams.focalSPL2(2));
        SPLs_lo2 = setdiff([10: XStimParams.SPLinc(2): 40], SPLs_hi2);
        if isempty(SPLs_lo2)
            XStimParams.focalProb2 = 1;
            set(H.focalProb2,'string','1');
        end
    case {'AMdepth  ', 'AMdepthdB'}     % focalSPLs gives actual depths for 1st and last focalDEPTH
        ...
        % REGIME 2
        Mdepth_lo2 = roundn(.1:.1:1,.1);
        Mdepth_hi2 = roundn(XStimParams.focalSPL2(1) : .1: XStimParams.focalSPL2(2),.1);
        Mdepth_lo2 = setdiff(Mdepth_lo2,Mdepth_hi2);
        if isempty(Mdepth_lo2)
            XStimParams.focalProb2 = 1;
            set(H.focalProb2,'string','1');
        end
    case {'AMperiod ', 'AMperiodB'}     % focalSPLs give indices to Mperiods for 1st and last focalPER
        ...
        % REGIME 2
        Mper_lo2 = [150 158 167 176 187 200 214 231 250 273 300 333 375 429 500 600 750 1000 1500];
        Mper_hi2 = Mper_lo2(XStimParams.focalSPL2(1) : XStimParams.focalSPL2(2));
        Mper_lo2 = setdiff(Mper_lo2,Mper_hi2);
        if isempty(Mper_lo2)
            XStimParams.focalProb2 = 1;
            set(H.focalProb2,'string','1');
        end
...
if isempty(find([1 2 3 6 7 8 9 10] == get(H.stim_type,'Value')))
    ...
    disp('Unsupported stimulus type for McSpace2 test; reset to BROADBAND');
end
...
switch McTestType
    case 'SPLtest  '     % focalSPLs gives dB-10 for 1st and last focalSPL
        XStimParams.test_type = 'McSpace2 STD';        str1 = 'SPL';
        disp('This is a STD McSpace2 test')
    case 'AMdepth  '     % focalSPLs gives actual depths for 1st and last focalDEPTH
        XStimParams.test_type = 'McSpace2 AMdepth lin';        str1 = 'AMdepth_lin';
        disp('This is a AMdepth lin McSpace2 test')
    case 'AMdepthdB'     % focalSPLs gives actual depths for 1st and last focalDEPTH
        XStimParams.test_type = 'McSpace2 AMdepth dB';        str1 = 'AMdepth_dB';
        disp('This is a AMdepth dB McSpace2 test')
    case 'AMperiod '     % focalSPLs give indices to Mperiods for 1st and last focalPER
        XStimParams.test_type = 'McSpace2 AMperiod lin';        str1 = 'AMperiod_lin';
        disp('This is a AMperiod lin McSpace2 test')
    case 'AMperiodB'     % focalSPLs give indices to Mperiods for 1st and last focalPER
        XStimParams.test_type = 'McSpace2 AMperiod dB';        str1 = 'AMperiod_dB';
        disp('This is a AMperiod dB McSpace2 test')
    otherwise
        XStimParams.test_type = ['McSpace2 using ' FN.stim];        str1 = 'file';
        disp(['This is a McSpace2 test using stimFN: ' FN.stim ])
end
...
eval(['save ' FN.temp_stim_path 'XStimParams_McSpace2_' str1 ' Temp_params;'])
...
%%%%%  make the stimulus
set(H.McSpace2_status,'BackgroundColor','yellow');
set(H.McSpace2_status,'ForegroundColor','blue');
set(H.McSpace2_status,'String','Status: building stimulus');
...
nEpochPts = round(nEpochmsecs * TDT.Fs/1000);       % all epochs have same nPts
nRegimePts = round(nRegimemsecs * TDT.Fs/1000);     % npts in REGIME1 & 2
nRegimePairs = round(nPts / sum(nRegimePts));       % nPairs of Regimes
nEpochs = round(nRegimePts / nEpochPts);            % nEpochs in REGIME1 & 2

nTotalEpochs = round(nPts / nEpochPts);
...
if get(H.stim_type,'Value')~=9
    ...
    if(exist1('H.McSpace2fig') & get(H.McSpace2_recorddata,'Value'))
        update_dataFN;
    end
    ...
    switch McTestType
        case 'SPLtest  '     % focalSPLs gives dB-10 for 1st and last focalSPL
            nPts = nTotalEpochs * nEpochPts;
            temp_ones = ones(nEpochPts,1);
            SPLs = 10:40; nSPLs = length(SPLs);
            
            ramppts = str2num(get(H.McSpace2_ramppts,'string'));
            if ramppts  ramp = 0:(ramppts-1);   end
            
            
            seq = zeros(nTotalEpochs,1); seqSPL = zeros(1,nPts);
            jepoch = 0;     % ongoing epoch count
            for iRegimePair = 1:nRegimePairs
                randEpochs1 = rand(nEpochs(1),1);       % for picking if hiProb or loProb
                randEpochs4 = rand(nEpochs(2),1);        
                if XStimParams.rndSPLs      % randomized order  only pseudo-random (no in-sequence repeats -
                    % order of hiProb
                    randEpochs2 = round(rand(nEpochs(1),1)*length(SPLs_hi)+.49999999999);  
                    randEpochs5 = round(rand(nEpochs(2),1)*length(SPLs_hi2)+.49999999999);  
                    % order of loProb
                    randEpochs3 = round(rand(nEpochs(1),1)*length(SPLs_lo)+.49999999999);
                    randEpochs6 = round(rand(nEpochs(2),1)*length(SPLs_lo2)+.49999999999);  
                elseif iRegimePair == 1    % each regimePair the same
                    % order of hiProb
                    randEpochs2 = repmat(randperm(length(SPLs_hi)),1,nEpochs(1))';
                    randEpochs5 = repmat(randperm(length(SPLs_hi2)),1,nEpochs(2))';
                    % order of loProb
                    randEpochs3 = repmat(randperm(length(SPLs_lo)),1,nEpochs(1))';
                    randEpochs6 = repmat(randperm(length(SPLs_lo2)),1,nEpochs(2))';
                end
                for iepoch = 1:nEpochs(1)       % REGIME 1
                    jepoch = jepoch+1;
                    if randEpochs1(iepoch) <= XStimParams.focalProb
                        seq(jepoch) = SPLs_hi(randEpochs2(iepoch));
                    else
                        seq(jepoch) = SPLs_lo(randEpochs3(iepoch));
                    end
                    st = (1+nEpochPts*(jepoch-1)); en = (nEpochPts*jepoch);
                    seqSPL(st:en) = seq(jepoch) * temp_ones;
                end
                for iepoch = 1:nEpochs(2)       % REGIME 2
                    jepoch = jepoch+1;
                    if randEpochs4(iepoch) <= XStimParams.focalProb2
                        seq(jepoch) = SPLs_hi2(randEpochs5(iepoch));
                    else
                        seq(jepoch) = SPLs_lo2(randEpochs6(iepoch));
                    end
                    st = (1+nEpochPts*(jepoch-1)); en = (nEpochPts*jepoch);
                    seqSPL(st:en) = seq(jepoch) * temp_ones;
                end
        ...
        case {'AMdepth  ', 'AMdepthdB'}     % focalSPLs gives actual depths for 1st and last focalDEPTH
            nCycles = XStimParams.mod_freq(1) .* nRegimePts*dt/1000;        % cycles in each REGIME
            cyclePts = TDT.Fs/XStimParams.mod_freq(1);
            tCycle = 1/TDT.Fs:1/TDT.Fs:1/XStimParams.mod_freq(1);
            % REGIME 1
            randCyclesHi = ceil(rand(nCycles(1),1)*length(Mdepth_hi));  
            randCyclesLo = ceil(rand(nCycles(1),1)*length(Mdepth_lo));
            randCycles = rand(nCycles(1),1);        
            % REGIME 2
            randCyclesHi2 = ceil(rand(nCycles(2),1)*length(Mdepth_hi2));  
            randCyclesLo2 = ceil(rand(nCycles(2),1)*length(Mdepth_lo2));
            randCycles2 = rand(nCycles(2),1);        
            ...
            for iRegime = 1:nRegimePairs
                ...
                for icycle = 1:nCycles(1)
                    ...
                    ACseq(1,jcycle) = Mi;
                    ind = (jcycle-1)*cyclePts+1:jcycle*cyclePts;
                    if strcmp(McTestType,'AMdepth  ')
                        E0(ind) = (1-Mi/2) + Mi/2 *E1;
                    elseif strcmp(McTestType,'AMdepthdB')
                        E0(ind) = 10.^((dBrange*Mi .*E1 - dBrange*Mi)/20);
                    end
                end
                for icycle = 1:nCycles(2)
                    jcycle=jcycle+1;
                    if randCycles2(icycle) <= XStimParams.focalProb2
                        Mi = Mdepth_hi2(randCyclesHi2(icycle));
                    else
                        Mi = Mdepth_lo2(randCyclesLo2(icycle));
                    end
                    ACseq(1,jcycle) = Mi;
                    ind = (jcycle-1)*cyclePts+1:jcycle*cyclePts;
                    if strcmp(McTestType,'AMdepth  ')
            ...
            trial_next(silence_len+1:silence_len+nPts) = trial_left(silence_len+1:silence_len+nPts) .* E0(:)';
            
        case {'AMperiod ', 'AMperiodB'}     % focalSPLs give indices to Mperiods for 1st and last focalPER
            % REGIME 1
            nCycles(1) = (nEpochPts .* nEpochs(1))/min1([Mper_hi Mper_lo]);      % max nCycles
            randCyclesHi = ceil(rand(nCycles(1),1)*length(Mper_hi));  
            randCyclesLo = ceil(rand(nCycles(1),1)*length(Mper_lo));
            randCycles = rand(nCycles(1),1);        
            % REGIME 2
            nCycles(2) = (nEpochPts .* nEpochs(2))/min1([Mper_hi2 Mper_lo2]);      % max nCycles
            randCyclesHi2 = ceil(rand(nCycles(2),1)*length(Mper_hi2));  
            randCyclesLo2 = ceil(rand(nCycles(2),1)*length(Mper_lo2));
            randCycles2 = rand(nCycles(2),1);        
            ...
            for iRegime = 1:nRegimePairs
                % REGIME 1
                icycle = 1;
                while nPts < iRegime * sum(nRegimePts) - nRegimePts(2)
                    jcycle = jcycle+1;
                    if randCycles(icycle) <= XStimParams.focalProb
                    ...
                    ind = nPts+1:nPts+Mperi;
                    if strcmp(McTestType,'AMperiod ')
                        E0(ind) = (1-Mdepth/2) + Mdepth/2 *E1;
                    elseif strcmp(McTestType,'AMperiodB')
                        E0(ind) = 10.^((dBrange*Mdepth .*E1 - dBrange*Mdepth)/20);
                    end
                    nPts = nPts+Mperi;
                    ACseq(1,jcycle) = Mperi;
                    icycle = icycle+1;
                end
                % REGIME 2
                icycle = 1;
                while nPts < iRegime * sum(nRegimePts)
                    jcycle = jcycle+1;
                    if randCycles2(icycle) <= XStimParams.focalProb2
                        Mperi = Mper_hi2(randCyclesHi2(icycle));
                    else
                        Mperi = Mper_lo2(randCyclesLo2(icycle));
                    end
                    tCycle = 1/TDT.Fs:1/TDT.Fs:Mperi/TDT.Fs;
                    E1 = sin(2*pi*(TDT.Fs/Mperi)*tCycle+XStimParams.mod_phase(1));
                    ind = nPts+1:nPts+Mperi;
        ...
        % save trial_left (basic noise) to disc
        if get(H.McSpace2_recorddata,'Value')
            noi = trial_left(silence_len+1:silence_len+nPts);
            eval(['save ' FN.data_path FN.data '_noi noi'])
            clear noi
        end
    ...
    if XStimParams.repeatmode         % save stim to file
        filename = [FN.stim_path 'McSpace2'];
    ...
    if(exist1('H.McSpace2fig') & ~exist1('H.McSpace2_finalspikeratefig'))
        H.McSpace2_finalspikeratefig = figure('Position',[700 20 550 500],...
            'Name','Space Test Spike Rate Plot',...
            'NumberTitle','off');
        H.McSpace2_spikeaxes = axes;
        colorbar
        set(H.McSpace2_spikeaxes,'Color','black');
    end
    
    %loop for reps
    repnum = 1;
    set(H.McSpace2_status,'String','Status: playing stimuli');
    set(H.McSpace2_status,'BackgroundColor','green');
    set(H.McSpace2_status,'ForegroundColor','white');
    
    while (exist1('H.McSpace2fig') & (repnum <= XStimParams.numreps))
        ...
        while (exist1('H.McSpace2fig') & (locnum <= nLocs) )
                    ...
                    switch McTestType
                        case 'SPLtest  '     % focalSPLs gives dB-10 for 1st and last focalSPL
                            ...
                            for iRegimePair = 1:nRegimePairs
                                randEpochs1 = rand(nEpochs(1),1);       % for picking if hiProb or loProb
                                randEpochs4 = rand(nEpochs(2),1);        
                                if XStimParams.rndSPLs      % randomized order
                                    % order of hiProb
                                    randEpochs2 = round(rand(nEpochs(1),1)*length(SPLs_hi)+.49999999999);  
                                    randEpochs5 = round(rand(nEpochs(2),1)*length(SPLs_hi2)+.49999999999);  
                                    % order of loProb
                                    randEpochs3 = round(rand(nEpochs(1),1)*length(SPLs_lo)+.49999999999);
                                    randEpochs6 = round(rand(nEpochs(2),1)*length(SPLs_lo2)+.49999999999);  
                                elseif 0    %iRegimePair == 1   % only pseudo-random (no in-sequence repeats)
                                    % order of hiProb
                                    randEpochs2 = repmat(randperm(length(SPLs_hi)),1,nEpochs(1))';
                                    randEpochs5 = repmat(randperm(length(SPLs_hi2)),1,nEpochs(2))';
                                    % order of loProb
            ...
            if(exist1('H.McSpace2fig') & ~isempty(spikes)) 
                spikes = spikes(ind);
            ...
            set(H.McSpace2_remtrials,'String',num2str(remtrials));
            locnum = locnum + 1;
            %pause(0);
            while t < ISI/1000
                t = toc;
        ...
        if 0
            ...
            figure(H.McSpace2_finalspikeratefig)
        ...
        set(H.McSpace2_remreps,'String',num2str(remreps));
        repnum = repnum + 1;
        pause(0);
    ...
    figure(H.McSpace2_finalspikeratefig)
    set(H.McSpace2_finalspikeratefig,'Name','Final Plot for Space Test');
    plotdiam1(XStimParams.locations, finalspikematrix);
    set(H.McSpace2_spikeaxes,'Color','black');
    ...
    set(H.McSpace2_status,'String','Status: results');
    set(H.McSpace2_status,'BackgroundColor','blue');
    
    set(H.exitMcSpace2,'Visible','on');
    set(H.resetMcSpace2,'Visible','on');
        ...
        case 'File'			
            if(~exist1('mod_from_file'))
                ...
                while length(mod_from_file) ~= Npts
                    ...
                    if(FN.mod_path ~= 0)
                        set(H.McSpace2_modfile,'String',[FN.mod_path FN.mod]);
                    end
                    fid = fopen(FN.mod,'r');
                    mod_from_file = fread(fid,inf,'float');
                    fclose(fid);
    ...
    flag = 0;
    %Check for pause by user
    while (exist1('H.McSpace2fig') & get(H.pauseMcSpace2,'Value'))
        pause(0);
        if(~exist1('H.McSpace2fig')) return; end         
        set(H.exitMcSpace2,'Visible','on');
        set(H.resetMcSpace2,'Visible','on');
        if(exist1('H.McSpace2fig') & get(H.resetMcSpace2,'Value') == 1)
            set(H.resetMcSpace2,'Value',0);
            set(H.pauseMcSpace2,'Value',0);
            Reset_McSpace2;    flag = 1;
            return;
        end
