...
% re-alterred version to use double buffering
% with writing first to files
%*******************************************************************************
...
if isempty(find([1 3 6 7 8 9] == get(H.stim_type,'Value')))
...
if Npts_totalplay <= TDT.buf_pts
    buf_pts = Npts_totalplay;
else
    buf_pts = TDT.buf_pts;
end

S232('allot16',BUF.L0,Npts_totalplay);
S232('allot16',BUF.R0,Npts_totalplay);

S232('allot16',BUF.L1,buf_pts);
S232('allot16',BUF.R1,buf_pts);
S232('allot16',BUF.L2,buf_pts);
S232('allot16',BUF.R2,buf_pts);
...
S232('PA4mute',1);					% HB1 adds 20 dB attenuation
S232('PA4mute',2);
...
remreps = 1;
set(H.space2_status,'String','Status: building stimuli');
...
while (exist1('H.space2fig') & (repnum <= 1))

    %Randomize the stimuli
    randseq = randperm(numtrials);
    
    trialnum = 1;
    while (exist1('H.space2fig') & (trialnum <= numtrials))
        ...
        switch get(H.stim_type,'Value')    
            case 1      % tone
                trial_left = MakeTone(TDT.Fs,XStimParams.curr_freq,XStimParams.curr_stimdur,0);
                trial_right = MakeTone(TDT.Fs,XStimParams.curr_freq,XStimParams.curr_stimdur,0);
            case 3
    ...
    if get(H.stim_type,'Value') ~= 9
        ...
        if(XStimParams.ildalone_flag == 1)
            ...
            itd_filt_left(1) = 0;   itd_filt_left(end) = 0;
            itd_filt_right(1) = 0;   itd_filt_right(end) = 0;
        ...
        if(XStimParams.itdalone_flag == 1)
            ...
            ila_filt_left(1) = 0;   ila_filt_left(end) = 0;
            ila_filt_right(1) = 0;   ila_filt_right(end) = 0;
        ...
        hrtf_left(1) = 0;   hrtf_left(end) = 0;
        hrtf_right(1) = 0;   hrtf_right(end) = 0;
        ...
        % make all-zero stimuli
        %filttrial_left = zeros(size(filttrial_left));
        %filttrial_right = zeros(size(filttrial_left));
        
        % save stimuli to disk
        if(exist1('H.space2fig'));
            S232('push16',filttrial_left,length(filttrial_left));
            S232('qpop16',BUF.L0);
            fname = ['space2_' str1 '.left_' num2str(hrtfdirmat(1,locind(randseq(trialnum)))) ...
                    '_' num2str(hrtfdirmat(2,locind(randseq(trialnum))))];
            evalstr = ['S232(''dama2disk16'',BUF.L0,' ...
                    [' ''' FN.temp_stim_path fname ''' '] ',0);'];
            eval(evalstr);
            temp_left = dama2pc(BUF.L0);
            S232('push16',filttrial_right,length(filttrial_right));
            S232('qpop16',BUF.R0);
            fname = ['space2_' str1 '.right_' num2str(hrtfdirmat(1,locind(randseq(trialnum)))) ...
                    '_' num2str(hrtfdirmat(2,locind(randseq(trialnum))))];
            evalstr = ['S232(''dama2disk16'',BUF.R0,' ...
                    [' ''' FN.temp_stim_path fname ''' '] ',0);'];
            eval(evalstr);
            temp_right = dama2pc(BUF.R0);
        end
        
        %Plot PSD
        if(exist1('H.space2fig') & ~exist1('H.space2_psdaxes'))
            figure(H.space2fig);
            H.space2_psdaxes = axes;
            set(H.space2_psdaxes,'Visible','off');
        end
        if(exist1('H.space2fig') & get(H.space2_plotpsd,'Value') == 1)
            axes(H.space2_psdaxes);
            set(H.space2_psdaxes,'Visible','on',...
                'Position',[0.1 0.07 0.8 0.2]);
            axis square
            [pl,f] = psd(temp_left,2048,TDT.Fs); [pr,f] = psd(temp_right,2048,TDT.Fs);
            H.space2_pl = plot(f,10*log10(abs(pl)),'g');
            hold on
            H.space2_pr = plot(f,10*log10(abs(pr)),'r');
            xlabel('Frequency (Hz)');
            ...
            set(H.space2_psdaxes,'YLim',[0 130]);
            hold off
            grid on
            pause(1);
        elseif(exist1('H.space2fig') & exist1('H.space2_pl') & get(H.space2_plotpsd,'Value') == 0)
            set(H.space2_psdaxes,'Visible','off');
            set(H.space2_pl,'Visible','off');
            set(H.space2_pr,'Visible','off');
        end
...
sound_left = zeros(1,Npts_totalplay);
sound_right = zeros(1,Npts_totalplay);
...
s_pt = 1;
e_pt = buf_pts;
S232('push16',sound_left(s_pt:(s_pt+buf_pts-1)),buf_pts);
S232('qpop16',BUF.L1);
S232('push16',sound_right(s_pt:(s_pt+buf_pts-1)),buf_pts);
S232('qpop16',BUF.R1);   
...
while (exist1('H.space2fig') & (repnum <= XStimParams.numreps))
 ...
 tic
    while (exist1('H.space2fig') & (trialnum <= numtrials) )
        ...
        fname = ['space2_' str1 '.left_' num2str(hrtfdirmat(1,locind(randseq(trialnum)))) ...
                '_' num2str(hrtfdirmat(2,locind(randseq(trialnum))))];
        eval(['fid = fopen(''' FN.temp_stim_path fname ''',''r'');'])
        sound_left = fread(fid,inf,'int16')';
        fclose(fid);

        fname = ['space2_' str1 '.right_' num2str(hrtfdirmat(1,locind(randseq(trialnum)))) ...
                '_' num2str(hrtfdirmat(2,locind(randseq(trialnum))))];
        eval(['fid = fopen(''' FN.temp_stim_path fname ''',''r'');'])
        sound_right = fread(fid,inf,'int16')';
        fclose(fid);
        ...
        %%%%% end of play buffer toggling loop  
