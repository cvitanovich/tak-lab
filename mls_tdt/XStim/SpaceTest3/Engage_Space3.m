...
%%%%%%% Doesn't seem to be totally ready Oct 14, 2012  (one_buf ??)
...
pt = 0;
while pt < npts_totalplay
    one_buf(BUF.L1,TDT.buf_pts);
    disp(['finished buf#1 : ' num2str(pt)])
    pt = pt+TDT.buf_pts;
    if pt < npts_totalplay
        one_buf(BUF.L2,TDT.buf_pts);
        disp(['finished buf#2 : ' num2str(pt)])
        pt = pt+TDT.buf_pts;
    end
