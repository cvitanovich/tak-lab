function [] = Engage_Two_Source_test()
...
%Engage_Two_source
% A TEST version to collect info on relative loudness of each source by
% itself
...
% get filter coefs and make Factor
Fs = 30000;
win = 30;
...
offset_el2 = XStimParams.offset_el2/2;
offset_az2 = XStimParams.offset_az2/2;
if ~XStimParams.ThreeStims
    ...
    for locnum = 1:size(XStimParams.locations,2)
        ...
        loc2 = max(find(hrtfdirmat(1,:) == (XStimParams.locations(1,locnum) - offset_el) &...
            hrtfdirmat(2,:) == (XStimParams.locations(2,locnum)) - offset_az));
        loc3 = max(find(hrtfdirmat(1,:) == (XStimParams.locations(1,locnum) + offset_el2) &...
            hrtfdirmat(2,:) == (XStimParams.locations(2,locnum)) + offset_az2));
...
while (exist1('H.Two_sourcefig') & (trialnum <= numtrials))
    ...
    if ~strcmp(XStimParams.mod_type,'None')
        Envelope = make_env(DUR, XStimParams.mod_type, XStimParams.mod_depth, XStimParams.mod_freq, XStimParams.mod_phase(1));
    ...
    %%%%% determine loudness of source 1 for each window and each frequency band
    frame = [silence_len length(source1_L)-silence_len2];
    Factormat = repmat(Factor',1,win);

    for iwin = 1:floor((diff(frame)+1)/win);
        % gammatone filter each stimulus  
        tempL = ERBFilterBankB(source1_L(silence_len+(iwin-1)*win+1:silence_len+iwin*win), fcoefs).* Factormat;		% has dimensions n_cF x length(noi)
        tempR = ERBFilterBankB(source1_R(silence_len+(iwin-1)*win+1:silence_len+iwin*win), fcoefs).* Factormat;
        ABL1(iwin,:) = (20*log10(std(tempR')) + 20*log10(std(tempL')))/2;
    end
    ...
    if ~strcmp(XStimParams.mod_type2,'None')
        Envelope = make_env(DUR, XStimParams.mod_type2, XStimParams.mod_depth2, XStimParams.mod_freq2, XStimParams.mod_phase(2));
    ...
    if(XStimParams.itdalone_flag == 1)
        if(trialnum == 1)
            ILAind = max(find(dir_ILA(1,:) == XStimParams.el2 & dir_ILA(2,:) == XStimParams.az2));
            if isempty('ILAind')
    ...
    if XStimParams.ildalone_flag | XStimParams.itdalone_flag
        ...
        %%%%% determine loudness of source2 for each window and each frequency band
    frame = [silence_len length(source1_L)-silence_len2];
    Factormat = repmat(Factor',1,win);

    for iwin = 1:floor((diff(frame)+1)/win);
        % gammatone filter each stimulus  
        tempL = ERBFilterBankB(source2_L(silence_len+(iwin-1)*win+1:silence_len+iwin*win), fcoefs).* Factormat;		% has dimensions n_cF x length(noi)
        tempR = ERBFilterBankB(source2_R(silence_len+(iwin-1)*win+1:silence_len+iwin*win), fcoefs).* Factormat;
        ABL2(iwin,:) = (20*log10(std(tempR')) + 20*log10(std(tempL')))/2;
    end

    %%%%%%%%%%%%%%%%%%
    if (XStimParams.ThreeStims == 1)
        % construct the 3rd stimulus
        if get(H.stim_type3,'Value') ~= 9
            source3_L = get_stim(XStimParams.freq(3));
        else
            source3_L = get_stim(FN.stim_path3,FN.stim3);
        end
        source3_R = source3_L;
        
        % modulate stim3
        if ~strcmp(XStimParams.mod_type3,'None')
            Envelope = make_env(DUR, XStimParams.mod_type3, XStimParams.mod_depth3, XStimParams.mod_freq3, XStimParams.mod_phase(3));
            source3_R = source3_R .* Envelope(:)';
        ...
        if get(H.stim_type3,'Value') ~= 9
            ...
            source3_R = ramp_sound(source3_R,TDT.Fs,ramp_time);
        end
        
        % remove any DCoffset
        source3_L = source3_L - mom(source3_L,1);
        source3_R = source3_R - mom(source3_R,1);
        ...
        if(XStimParams.itdalone_flag == 1)
            if(trialnum == 1)
                ILAind = max(find(dir_ILA(1,:) == XStimParams.el3 & dir_ILA(2,:) == XStimParams.az3));
                if isempty('ILAind')
                    disp('Could not find ILA location #3 in HRTF file');
                    return
                end
                if FN.HRTFfiletype(2) == 1
                    eval(['ila_filt_left3 = mtlrch(''' FN.ILA_path FN.ildalone ''', ' num2str(ILAind * 2-1) ');']);
                    eval(['ila_filt_right3 = mtlrch(''' FN.ILA_path FN.ildalone ''', ' num2str(ILAind * 2) ');']);
                else
        ...
        if(XStimParams.space_flag == 1)
            if FN.HRTFfiletype(1) == 1
                hrtf_left = mtlrch([FN.space_path FN.space],(2*locind(3,randseq(trialnum)))-1);
                hrtf_right = mtlrch([FN.space_path FN.space],2*locind(3,randseq(trialnum)));
            else
        ...
        elseif(XStimParams.ABLalone_flag == 1)
            if FN.HRTFfiletype(1) == 1
                tempL = mtlrch([FN.space_path FN.space],(2*locind(3,randseq(trialnum)))-1);
                tempR = mtlrch([FN.space_path FN.space],2*locind(3,randseq(trialnum)));
            else
        ...
        elseif(XStimParams.ildalone_flag == 1)
            if FN.HRTFfiletype(1) == 1
                hrtf_left = mtlrch([FN.ILA_path FN.ildalone],(2*locind(3,randseq(trialnum)))-1);
                hrtf_right = mtlrch([FN.ILA_path FN.ildalone],2*locind(3,randseq(trialnum)));
            else
        ...
        elseif(XStimParams.itdalone_flag == 1)
            if FN.HRTFfiletype(1) == 1
                hrtf_left = mtlrch([FN.ITA_path FN.itdalone],(2*locind(3,randseq(trialnum)))-1);
                hrtf_right = mtlrch([FN.ITA_path FN.itdalone],2*locind(3,randseq(trialnum)));
            else
    ...
    if XStimParams.ildalone_flag | XStimParams.itdalone_flag
            ABAval = 0.5*(mom(source2_L,2) + mom(source2_R,2));
            scalefact = TDT.scalevalue/ABAval;
            source3_L = round(scalefact*source3_L);
            source3_R = round(scalefact*source3_R);
        end
        % add the three weighted sources
        trial_left = (source1_L * XStimParams.factor + source2_L * XStimParams.factor2 + source3_L * XStimParams.factor3)/3;
        trial_right = (source1_R * XStimParams.factor + source2_R * XStimParams.factor2 + source3_R * XStimParams.factor3)/3;
        %%%%%%%%%%%%%%%%%
    else      % only use two stims
        trial_left = (source1_L * XStimParams.factor + source2_L * XStimParams.factor2)/2;
        trial_right = (source1_R * XStimParams.factor + source2_R * XStimParams.factor2)/2;
        %%%%%%%%%%%%%%%%%
    ...
    %%%%% save ABLs to disk with name of loc1
    if(exist1('H.Two_sourcefig'));
        fname = ['ABL_' num2str(hrtfdirmat(1,locind(1,randseq(trialnum)))) ...
                '_' num2str(hrtfdirmat(2,locind(1,randseq(trialnum))))];
        evalstr = ['save ' stimuli_dir fname ' ABL*'];
        eval(evalstr);
    end
