...
elseif(ephoneflag == 1 & XStimParams.picker_flag == 0)
...
elseif(ephoneflag == 1 & XStimParams.picker_flag == 1)
...
if(exist1('H.fig') & ephoneflag & XStimParams.picker_flag)
    S232('PD1npts',1,(silence_len + DUR*(round(TDT.Fs/1000))) + TDT.itdfiltlen  + ...
        TDT.ephonefiltlen + TDT.hrtffiltlen);
elseif(exist1('H.fig') & ephoneflag == 1 & ~XStimParams.picker_flag)
    S232('PD1npts',1,(silence_len + DUR*(round(TDT.Fs/1000))) + TDT.itdfiltlen  + TDT.ephonefiltlen);
elseif(exist1('H.fig') & ~ephoneflag)
    S232('PD1npts',1,(silence_len + DUR*(round(TDT.Fs/1000))) + TDT.itdfiltlen);
end

%Load Earphone filters
if(exist1('H.ablfig') & get(H.ephoneuseit,'Value')) %Use earphone filters
...
while (exist1('H.ablfig') & (trialnum <= numtrials))
    ...
    if ~XStimParams.picker_flag       % 3/7/07 multiply by freefield_Xfactors
   ...
   %Add in the leading silent period
    ...
    % pad for filters
    if(XStimParams.picker_flag & ephoneflag)
        trial_left = [trial_left zeros(1,TDT.ephonefiltlen) zeros(1,hrtffiltlen)];
        trial_right = [trial_right zeros(1,TDT.ephonefiltlen) zeros(1,hrtffiltlen)];
    elseif(ephoneflag)
    ...
    if XStimParams.picker_flag
        ...
        locind = max(find(hrtfdirmat(1,:) == XStimParams.locations(1,1) & hrtfdirmat(2,:) == XStimParams.locations(2,1)));
        % get HRTFs
        if FN.HRTFfiletype(1,2) == 1
