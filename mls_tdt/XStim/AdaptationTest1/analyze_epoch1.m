        ...
        if ~isempty(ind1)
            ...
            for iEpoch = 2:nEpochs
                ...
                if ~isempty(ind3)
                    ...
                    nEspikes = length(temp_times);
                    while nEspikes > maxEspikes            % underestimated maxEspikes, therefore need to add on here
                        Spikes{indEpoch}.spiketimes = [Spikes{indEpoch}.spiketimes zeros(nReps,(nEspikes- maxEspikes))];
                        maxEspikes = nEspikes;
                    end
