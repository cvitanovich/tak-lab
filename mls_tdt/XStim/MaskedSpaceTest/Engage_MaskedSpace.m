...
ramp_usecs = 1000 * 5;                              % was  * 10 until June 25, 2009
...
S232('dpush',255);                  % extra coefs for noG
S232('value',0.0);
S232('cat');
...
S232('scale',1.732 * scaleFactor * (10^(XStimParams.mask_add_dB /20)));         % Jun08,09 added 1.732 to norm by mom2
...
S232('scale',1.732 * scaleFactor * 10^(XStimParams.probe_add_dB /20));         % Jun08,09 added 1.732 to norm by mom2
...
for iRep = 1:XStimParams.numreps
    ...
    for iLoc = 1:nLocs+1
        ...
        %%%%%%% delay
        while (toc < (.15+nptsTotalPlay/TDT.Fs  + (XStimParams.test_ISI-100)/1000))   end
        
        if (iLoc>1)
            S232('PD1stop',1);
            m110dx( C_.STOP);
            spikes(iLoc-1,:) = m110dx( C_.DATA, nSpikes)';          % error here for first use on April29,09
        end
        if pause_check    return; end
    ...
    for iLoc = 1:nLocs
        ...
        if ~isempty(temp)
            ...
            temp = temp(ind0)/(1000/mii_us_per_sample); nspikes = length(temp);
            nspikesProbe = length(find(temp>XStimParams.curr_stimdur & temp<XStimParams.curr_stimdur+XStimParams.curr_stimdur2));
            spikes_trial = [spikes_trial;temp];
            EL_trial = [EL_trial; tempdir(1,iLoc)* ones(size(temp))];
            AZ_trial = [AZ_trial; tempdir(2,iLoc)* ones(size(temp))];
            repnum_trial = [repnum_trial; iRep * ones(size(temp))];
            Nspikes = [Nspikes; nspikes * ones(size(temp))];
        end
        finalspikematrix(randseq(iLoc)) = finalspikematrix(randseq(iLoc)) + nspikesProbe;
    end
...
title(['Maximum Activity to Probe at EL = ' num2str(hrtfdirmat(1,locmaxspikes)) ...
