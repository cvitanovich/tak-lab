function [IRout, IRin, ALTout] = altmagIR5 (IRin, octlist, Fact, ALTin, pts)
...
%       pts deterines the octave bandwidth for alteration (e.g. pts =1
%       means 1/12th octave, pts = 2 means 1/6th octave bandwidth)
%
% modified version to only change non-zero parts of IR
% allows multiple frequency bands

if nargin < 5 | isempty(pts)        pts = 1;    end
if nargin < 4 | isempty(ALTin)      ALTin = []; end
if nargin < 3 | isempty(Fact)       Fact = 10;	end
if nargin < 2 | isempty(octlist)	octlist = [13:25] ; end
...
% octave bandwidth for changing depends on pts
for i = 1:pts:length(octlist)
    if (i+pts-1) <= length(octlist)
        temp(ind(octlist(i:(i+pts-1)))) = ones(1,pts).*rand(1)*2-1;
    else
        temp(ind(octlist(i):octlist(i)+pts-1)) = ones(1,pts).*rand(1)*2-1;   
    end
end
...
% reset anything higher or lower to 0
ind = [1:pt0-2 pt1+2:length(temp)];
...
if ~isempty(ALTin)                              % use alteration from argin
    temp = ALTin;
end
