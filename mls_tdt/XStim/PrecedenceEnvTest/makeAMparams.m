function [params,usedBank] = makeAMparams(randBank,usedBank)

% [params,usedBank] = makeAMparams(randBank,usedBank);
%
...
for iLoc=1:2
    for icF = 1:n_cF
        for idelay = 1:n_delay
            ind0 = find(usedBank(:,itest)==0);          % all available rows of randBank for this test
            ind1 = ind0(ceil(rand(1)*length(ind0)));    % which row to use for this test
            usedBank(ind1,itest)=usedBank(ind1,itest)+1;                     % mark it as used
