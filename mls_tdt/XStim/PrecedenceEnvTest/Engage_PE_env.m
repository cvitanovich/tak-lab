...
%
% June26_12 changed makeAMparams to makeAMparams1 to better use randBank
%           ATTN: added flatten of Noise1 and Noise2 to
%           filteredstimuliAM_01 and _02
%           changed method of updating TDT to current stimuli
...
if exist('e:\kip\MATLAB\mls_tdt\XStim\PrecedenceEnvTest\randBank.mat','file') & numtests <=50
    load 'e:\kip\MATLAB\mls_tdt\XStim\PrecedenceEnvTest\randBank.mat'
else
    randBank = ceil(rand(numtests,5) * 10000000);
    save e:\kip\MATLAB\mls_tdt\XStim\PrecedenceEnvTest\randBank2.mat randBank
end

% for FROZEN NOISE
if 1
    randBank = repmat(ceil(rand(1,5) * 10000000),50,1);
end

%loop for reps (of blocks)
while (exist1('H.PE_envfig') & (irep <= XStimParams.numreps))
    % make params for this rep  (changed to use makeAMparams1 on 6/26/12)
    [AMparams{irep} usedBank] = makeAMparams1(randBank,usedBank,irep);
    ...
    while itest <= numtests
        ...
        if irep <= XStimParams.numreps
            ...
            if 0        % trying replacement of this 6/12
                S232('dropall');
                ...
                %Make play sequence buffers 
                % to ID which sound buffer (1,2,3) comes from which location
                % RF
                S232('dropall');
                S232('dpush',1);
                S232('make',0,AMparams{irep}(test,11));          
                S232('qpoppart16',BUF.playseq1,0);
                % anti RF
                S232('dropall');
                S232('dpush',1);
                S232('make',0,AMparams{irep}(test,12));
                S232('qpoppart16',BUF.playseq2,0);
            else        % new way 6/12
                S232('dropall');
                if AMparams{irep}(test,11) == 1
                    S232('push16',[zeros(1,silence_len) Noise1 zeros(1,silence_len2+ 255*2)],nPts_totalplay);
                elseif AMparams{irep}(test,11) == 2
                    S232('push16',[zeros(1,silence_len) Noise2 zeros(1,silence_len2+ 255*2)],nPts_totalplay);
                else
                    S232('push16',[zeros(1,silence_len) Zeros1 zeros(1,silence_len2+ 255*2)],nPts_totalplay);
                    str1 = 'none';
                end
                S232('qpop16',BUF.snd1);
                
                if AMparams{irep}(test,12) == 1
                    S232('push16',[zeros(1,silence_len) Noise1 zeros(1,silence_len2+ 255*2)],nPts_totalplay);
                elseif AMparams{irep}(test,12) == 2
                    S232('push16',[zeros(1,silence_len) Noise2 zeros(1,silence_len2+ 255*2)],nPts_totalplay);
                else
                    S232('push16',[zeros(1,silence_len) Zeros1 zeros(1,silence_len2+ 255*2)],nPts_totalplay);
                    str2 = 'none';
            ...
            str1 = [num2str(AMparams{irep}(test,11)) '   '];
            str2 = num2str(AMparams{irep}(test,12));
            disp(['loc1:   ' str1 '  loc#2:   ' str2 '   test# ' num2str(test)])
            pause(.1)
        end             % end of making next stim & loading DSPs
        
        while toc < (XStimParams.curr_stimdur + 1000)/1000     end
        tic
        ...
        if ~isempty(spikes) 
            ...
            if AMparams{irep}(test,11)<3
                EL_trial = dir(1,loc2use(AMparams{irep}(test,11)))* Ones1;
                AZ_trial = dir(2,loc2use(AMparams{irep}(test,11)))* Ones1;
            else
                EL_trial = 999* Ones1;
                AZ_trial = 999* Ones1;
            end
            irep_trial = irep * Ones1;
            itest_trial = test * Ones1;
            Nspikes = length(spikes) * Ones1;
