...
now:
mex -g play2_record2D2_alex3.c c:\kip\code\play_record\play0.c c:\kip\code\play_record\play0_record0.c c:\tdt\S232\VC\S2drv32c.lib
...
double_buffer( InfoStruct *recInfo, float *leadSnd, float *scaleVals, float *lagSnd, float *locations, float *azimuths, int nSignal )
{
FILE	*fptr;
long	seekpos = 0;
...
do
{
	...
	if (cnt==recInfo->ISI)
	{
		...
		memset(str,'\0',sizeof(str));
		xy[0] = Signalcnt+1;
        xy[1] = azimuths[loc-1]; /* determine azimuth of lag */
	...
	{		// downloading  #1 recordbuffers
		...
		if(SignalPlayFlag) {
			...
			for(i=0; i<DEC_PTS; i++) {
				memset(str,'\0',sizeof(str));
				n=sprintf(str,"tmp(%i) = %.5f;",i+1,pdrBuffer[i]-pdrBuffer[0]);
				mexEvalString(str);
			}
			memset(str,'\0',sizeof(str));
...
{ 
	...
	double		dbl;
	float		*leadSnd, *lagSnd, *scaleVals, *locations, *azimuths;
...
// check nptsTotalPlay = (n * buf_pts)
	...
	double_buffer( &recInfo, leadSnd, scaleVals, lagSnd, locations, azimuths, nsignal);
