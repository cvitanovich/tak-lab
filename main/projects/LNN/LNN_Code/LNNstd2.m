function [rbest phi_best] = LNNstd2(time,Fs,s)
...
%mu0 = mean(noise);
%n0= length(noise);
r = std(noise)/std(abs(hilbert(noise))); 
...
while flag == 1
    ...
    noise = noise - mean(noise);
    %noise = noise./std(noise);
    old = std(noise)/std(abs(hilbert(noise))); 
    %var(noise)/var(abs(hilbert(noise))); %(1/n0)*sum((noise - mu0).^4) / ((1/n0)*sum((noise - mu0).^2))^2;
    ...
    %mu0 = mean(noise);
    %n0 = length(noise);
    new = std(noise)/std(abs(hilbert(noise)))
%var(noise)/var(abs(hilbert(noise))) %(1/n0)*sum((noise - mu0).^4) / ((1/n0)*sum((noise - mu0).^2))^2;
    toc
    if new >= (old + 0.00001)
    else
...
rbest = new;
    ...
    parfor p=1:sz
        ...
        magmat = repmat(mag,1,num);
        phimat = repmat(phi,1,num);
        tmp = zeros(len,num);
        tmp2 = ones(num,1);
        tmp2 = delta * diag(tmp2);
        tmp(minfreq+(p-1)*100:minfreq + (p-1)*100 + num - 1,:) = tmp2; % maxfreq + 1 - freq: maxfreq + 1 -1
        %noi = zeros(length(phi),1);
        noi = real( ifft( magmat .* ((cos(phimat+tmp)) + (sqrt(-1)*sin(phimat+tmp))) ));
        noi = noi - repmat(mean(noi),size(noi,1),1);
        %noi = noi./std(noi);
        %        noi = noi/(abs(max(noi)));
        %         noi = abs(hilbert(noi));
%         mu = mean(noi);
%         n = length(noi);
        f1{p} = (std(noi)/std(abs(hilbert(noi))))'; 
	%var(noi)/var(abs(hilbert(noi))); 
	%f1 = (1/n)*sum((noi - mu).^4) / ((1/n)*sum((noi - mu).^2))^2;
...
%         noi = abs(hilbert(noi));
        %mu = mean(noi);
        %n = length(noi);
        f2{p} = (std(noi)/std(abs(hilbert(noi))))'; 
        %var(noi)/var(abs(hilbert(noi))); 
        %f2 = (1/n)*sum((noi - mu).^4) / ((1/n)*sum((noi - mu).^2))^2;
    ...
    for w = 1:sz
        ...
        g(minfreq+(w-1)*100:minfreq + (w-1)*100 + nm - 1) = (f1{w}-f2{w})/(2*delta); % maxfreq + 1 - fact
    end
    ...
    parfor i = 1:201
        ...
        noi = noi - mean(noi);
        %noi = noi./std(noi);
        r(i) = std(noi)/std(abs(hilbert(noi)));
    end
...
%        mu = mean(noi);
%        n = length(noi);
        newvals(i2) = std(noi)/std(abs(hilbert(noi))); 
%var(noi)/var(abs(hilbert(noi)));
%        newvals(i2,2) = x;
 %       newvals(i2,3) = y;
    end
