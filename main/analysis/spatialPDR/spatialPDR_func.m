...
function parse_trials
    ...
    for i = 1:length(A.trials.types)
        tmp=find(A.SPL_list==A.trials.types(i));
        A.trials.test{i}.spl = [num2str(A.SPL_list(tmp))];
...
%     leg{3} = 'Hab A.trials (Last Half)';
    ...
    for i = 1:tt_types
        ...
        leg{i+1} = [A.trials.test{i}.spl ' dB'];
...
function plot_zscores
        ...
        xes(1:size(A.trials.hab.zscores,2)) = -50*ones(size(A.trials.hab.zscores,2),1);
        ...
        for j = 1:length(A.trials.types)
            xes(len2:(len2+size(A.trials.test{j}.zscores,2)-1)) = str2num(A.trials.test{j}.spl)*ones(size(A.trials.test{j}.zscores,2),1);
            zes(len2:(len2+size(A.trials.test{j}.zscores,2)-1)) = A.trials.test{j}.zscores;
            pos = A.trials.test{j}.spl;
...
%         grid on; axis tight;
        set(gca,'Color',[.7 .7 .7]);
        xlabel ('SPL'); ylabel ('z-scores');
         title (['z-scores for each SPL tested']);
...
A.lowerCI(i) = AUC(2);
A.upperCI(i) = AUC(3);
title(['SPL = ' A.trials.test{i}.spl ' dB, PC=' num2str(AUC(1))]);
drawnow;
