function [X,Xtime] = whnoise(minfreq,maxfreq,Fs,duration,phi)
%A script to produce noise with a flat spectrum between specified frequencies.
%[X,Xtime] = whnoise(minfreq,maxfreq,Fs,duration,phi)
%adapted from Kip Keller w_rnd.m


if (nargin < 3) Fs = 30000; end
if (nargin < 4) duration = .1; end
if (nargin < 5) phase = (rand(duration*Fs,1) - 0.5) * (2*pi);
else
phase = phi;
end
