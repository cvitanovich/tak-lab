function [resp, dirs, full_rs, new_f_ax, new_ild_ax] = if2space_orig(ildspechrtf, specfreqs, locs, ildfreq_data, f_ax, ild_ax, checklocs); 
...
ildspechrtf(ildspechrtf>=maxild) = maxild*zeros(size(ildspechrtf(ildspechrtf>=maxild)));
ildspechrtf(ildspechrtf<=minild) = minild*zeros(size(ildspechrtf(ildspechrtf<=minild)));
...
infreqs = specfreqs<maxfreq & specfreqs>minfreq;
ildspechrtf = ildspechrtf(:,infreqs);
freq_axis_hrtf = specfreqs(infreqs);
