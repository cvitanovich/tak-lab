function err = errorfun_spectralintegration1(V,freq_axis,Tonal_meansurf,BP_meansurf)
...
for num_freq = 1:length(freq_axis)
   ...
   Tonal_meansurf_new(num_freq,:) = ...
      mean( f_weights(index_1:index_2,:).^V .* (Tonal_meansurf(index_1:index_2,:)),1);
end

err = mean(mean(abs(Tonal_meansurf_new - BP_meansurf)));
