...
for cell_num = 19:20
   ...
   V = [1 1 0 0 10 10]; %Frequency weights, ILDopt_shifts, ILD tuning widths
   [V,fval,exitflag,output] = fminsearch('errorfun_spectralintegration13',...
      V,...
      Options,...
      IA_fullspec,...
      ILD_piece1,...
...
max_activity = max(IA_fullspec)/2;
...
%Interpolate the missing values in the ILDAlone measurement
   ...
   title(str2mat([Model ' Predicted ILDAlone'],['r^{2} = ', num2str(cc(1,count))],['ILD width = (', ...
         num2str(vars(count,5:6)), ')']),'FontSize',8);
   if(exist('space_contour'))
   hold on;
   plot(space_contour(1,:),space_contour(2,:),linecolor,'LineWidth',1.5);
   hold off;
