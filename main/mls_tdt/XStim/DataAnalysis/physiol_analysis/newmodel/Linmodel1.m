...
for cell_num = 19:24
   ...
   for band = 1:2
      ia_test(band) = (2*band);
      IA_bandwidths(band,:) = Neuron{cell_num}.ia_meansurf{ia_test(band)};
   end
   %minimize the model error
   Options = optimset('Display','iter','MaxIter',500,'TolFun',1e-8,'TolX',1e-10,'MaxFunEvals',1000);
   V = [0 1]; %coefficients for model
   [V,fval,exitflag,output] = fminsearch('errfun_specint5',V,Options,...
      IA_fullspec,IA_bandwidths);
   [err,result] = errfun_specint5(V,IA_fullspec,IA_bandwidths);
   
   IA_linpredict = result;
   IA_predictstore(cell_num,:) = IA_linpredict;
...
%Interpolate the missing values in the ILDAlone measurement
   ...
   if(any(strcmp(fieldnames(Neuron{cell_num}),'tia_diamond')));
   tia_h = subplot(3,3,5);
   ...
   if(cell_num <= 20)
      ...
      bandlimits = [Neuron{cell_num}.ia_header{ia_test(1)}(16) Neuron{cell_num}.ia_header{ia_test(1)}(17)];
   end
   ...
   if(cell_num <= 20)
      ...
      bandlimits = [Neuron{cell_num}.ia_header{ia_test(2)}(16) Neuron{cell_num}.ia_header{ia_test(2)}(17)];
   end
   title(str2mat('Measured ILDAlone',['freq band = (' num2str(bandlimits(1)), ',',...
         num2str(bandlimits(2)), ')']),'FontSize',8);
   hold on;
