...
%Directories & files
cache_directory = ['c:\michael\matlab\scripts\physiol_analysis\' num2str(bird_number) 'analysis\' ...
      num2str(bird_number) '_cache\'];
hrtf_directory  = ['c:\michael\HRTF_files\' num2str(bird_number) '\'];
hrtf_file       = ['out9be'];
hrtf_file2		 = ['out29be'];
get_HRTF = 0;
get_ITDmatrix = 0;
...
load 'c:\michael\matlab\save\Neuron'
...
for cell_num = 19:20
   ...
   V = [ones(1,8) 10 10]; %ILD tuning widths for model
   [V,fval,exitflag,output] = fminsearch('errorfun_spectralintegration10',...
      V,...
      Options,...
      IA_fullspec,...
      ILD_piece1,...
...
%Interpolate the missing values in the ILDAlone measurement
   ...
   title(str2mat([Model ' Predicted ILDAlone'],['r^{2} = ', num2str(cc(1,count))],['ILD widths = (', ...
         num2str(vars(count,9)), ' ', num2str(vars(count,10)), ')']),'FontSize',8);
   if(exist('space_contour'))
   hold on;
   plot(space_contour(1,:),space_contour(2,:),linecolor,'LineWidth',1.5);
   hold off;
