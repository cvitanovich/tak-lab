...
for cell_num = 22:24
   ...
   %Get frequency weights by taking the activity within a range of the ILD values of the best
   %ILDAlone spectrum
   for freq_num = 1:length(new_if_freqaxis)
      band_num = find(freq_bands(:,1) <= new_if_freqaxis(freq_num) & ...
         freq_bands(:,2) >= new_if_freqaxis(freq_num));
      if(~isempty(band_num))
         band_num = max(band_num);
         ILD_opt = ILD_matrix_focus(freq_num,IA_bestloc_ind);
         [y,if_ildindex] = min(abs(Neuron{cell_num}.tif_ildaxis - ILD_opt));
         if_ildindex = max(if_ildindex);
         temp_weight(freq_num,band_num) = new_if_surface(freq_num,if_ildindex);
      end
   end
   
   for band_num = 1:size(freq_bands,1)
      if(isempty(dezero(temp_weight(:,band_num))))
         freq_weight(band_num) = 0;
      else
         freq_weight(band_num) = mean(dezero(temp_weight(:,band_num)));
      end
   end
   
   %minimize the model error
   Options = optimset('Display','iter','MaxIter',1000,'TolFun',0.002,'TolX',0.01,'MaxFunEvals',24000);
   V = [10*ones(1,size_freq_bands) ...
      zeros(1,size_freq_bands) 0.6]; %ILD tuning widths & ILD peak shifts for model
   [V,fval,exitflag,output] = fminsearch('errfun_specint4',...
      V,...
      Options,...
   ...
   [err,result,V,freq_weight] = errfun_specint4(V,...
      Neuron{cell_num}.ia_meansurf{1},...
      new_if_surface,...
      new_if_freqaxis,...
      Neuron{cell_num}.tif_ildaxis,...
