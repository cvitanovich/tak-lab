%Two_source: the GUI for Two_source tests

%*******************************************************************************
%	The Graphical User Interface
%*******************************************************************************

GUI.locations1 = [];
GUI.locations2 = [];
XStimParams.reset_flag = 0;
XStimParams.bandwidth = [0.3333 0.3333 0.3333];
XStimParams.bandlimits = [2000 11000 2000 11000 2000 11000];

F.pos_incr = 20;
F.tot_ht = 0;
F.ind_ht = 20; 
%Figure window
if(~exist1('H.searchfig') &...
        ~exist1('H.ablfig') &...
        ~exist1('H.itdfig') &...
        ~exist1('H.ildfig') &...
        ~exist1('H.ildfreqfig') &...
        ~exist1('H.Two_sourcefig'))
    temp = get(H.fig,'Position');
    F.Two_source_l = temp(3) - 300; 
    F.Two_source_wd = 400; 
    F.Two_source_ht = 900;
    H.Two_sourcefig = figure('Units','pixels',...
        'Position',[F.Two_source_l 100 F.Two_source_wd F.Two_source_ht],...
        'Name','TWO-source Space or ILDAlone Measurement',...
        'NumberTitle','off');
    F.tot_ht = F.tot_ht + F.ind_ht;
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht F.Two_source_wd F.ind_ht],...
        'ForegroundColor','blue',...
        'FontWeight','bold',...
        'String', 'TWO-sources');
    %reminder
    F.tot_ht = F.tot_ht + F.ind_ht;
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht F.Two_source_wd F.ind_ht],...
        'ForegroundColor','blue',...
        'String', 'Earphone Inverse Filters loaded separately to DSPs');
    
    %Specify HRTF and Location files
    F.tot_ht = F.tot_ht + F.ind_ht;
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht F.Two_source_wd F.ind_ht],...
        'BackgroundColor','blue',...
        'ForegroundColor','white',...
        'FontWeight','bold',...
        'String', 'Specify Files');
    
    %Fully-cued HRTF file
    F.tot_ht = F.tot_ht + F.ind_ht;
    H.spacefilepb = uicontrol('Parent',H.Two_sourcefig,...
        'Style','pushbutton',...
        'Units','pixels',...
        'Position',[2 F.Two_source_ht-F.tot_ht 195 F.ind_ht],...
        'BackgroundColor','red',...
        'ForegroundColor','white',...
        'FontWeight','bold',...
        'String', 'Select Fully-cued Filter File',...
        'Callback','setInfo_Two_source');
    
    uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[200 F.Two_source_ht-F.tot_ht 120 F.ind_ht],...
        'String', 'Fully-cued Test');
    H.spaceflag = uicontrol('Parent',H.Two_sourcefig,...
        'Style','checkbox',...
        'Units','pixels',...
        'Position',[315 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'Value',(XStimParams.space_flag),...
        'Callback','setInfo_Two_source');
    F.tot_ht = F.tot_ht + F.ind_ht;
    H.spacefilehdr = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Position',[0 F.Two_source_ht-F.tot_ht 120 F.ind_ht],...
        'String', 'Fully-cued HRTF File:');
    H.spacefile = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Position',[120 F.Two_source_ht-F.tot_ht F.Two_source_wd-120 F.ind_ht],...
        'HorizontalAlignment','left',...
        'ForegroundColor','blue',...
        'String', FN.space_std);
    uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[200 F.Two_source_ht-F.tot_ht 120 F.ind_ht],...
        'String', 'ABL-alone Test');
    H.ABLaloneflag = uicontrol('Parent',H.Two_sourcefig,...
        'Style','checkbox',...
        'Units','pixels',...
        'Position',[315 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'Value',(XStimParams.ABLalone_flag),...
        'Callback','setInfo_Two_source');
    
    %ILDAlone filter file
    F.tot_ht = F.tot_ht + F.ind_ht;
    H.ildalonepb = uicontrol('Parent',H.Two_sourcefig,...
        'Style','pushbutton',...
        'Units','pixels',...
        'Position',[2 F.Two_source_ht-F.tot_ht 195 F.ind_ht],...
        'BackgroundColor','red',...
        'ForegroundColor','white',...
        'FontWeight','bold',...
        'String', 'Select ILDAlone Filter File',...
        'Callback','setinfo_Two_source');
    
    uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[200 F.Two_source_ht-F.tot_ht 120 F.ind_ht],...
        'String', 'ILDAlone Test');
    H.ildaloneflag = uicontrol('Parent',H.Two_sourcefig,...
        'Style','checkbox',...
        'Units','pixels',...
        'Position',[315 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'Value',XStimParams.ildalone_flag,...
        'Callback','setInfo_Two_source');
    F.tot_ht = F.tot_ht + F.ind_ht;
    H.ildalonefilehdr = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Position',[0 F.Two_source_ht-F.tot_ht 120 F.ind_ht],...
        'String', 'ILDAlone File:');
    H.ildalonefile = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Position',[120 F.Two_source_ht-F.tot_ht F.Two_source_wd-120 F.ind_ht],...
        'HorizontalAlignment','left',...
        'ForegroundColor','blue',...
        'String', FN.ildalone_std);
    
    %ITDAlone filter file
    F.tot_ht = F.tot_ht + F.ind_ht;
    H.itdalonepb = uicontrol('Parent',H.Two_sourcefig,...
        'Style','pushbutton',...
        'Units','pixels',...
        'Position',[2 F.Two_source_ht-F.tot_ht 195 F.ind_ht],...
        'BackgroundColor','red',...
        'ForegroundColor','white',...
        'FontWeight','bold',...
        'String', 'Select ITDAlone Filter File',...
        'Callback','setinfo_Two_source');
    
    uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[200 F.Two_source_ht-F.tot_ht 120 F.ind_ht],...
        'String', 'ITDAlone Test');
    H.itdaloneflag = uicontrol('Parent',H.Two_sourcefig,...
        'Style','checkbox',...
        'Units','pixels',...
        'Position',[315 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'Value',XStimParams.itdalone_flag,...
        'Callback','setInfo_Two_source');
    F.tot_ht = F.tot_ht + F.ind_ht;
    H.itdalonefilehdr = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Position',[0 F.Two_source_ht-F.tot_ht 100 F.ind_ht],...
        'String', 'ITDAlone File:');
    H.itdalonefile = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Position',[100 F.Two_source_ht-F.tot_ht F.Two_source_wd-100 F.ind_ht],...
        'HorizontalAlignment','left',...
        'ForegroundColor','blue',...
        'String', FN.itdalone_std);
       
    F.tot_ht = F.tot_ht + F.ind_ht + F.ind_ht;
    %Other Stimulus Parameters
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht F.Two_source_wd F.ind_ht],...
        'BackgroundColor','blue',...
        'ForegroundColor','white',...
        'FontWeight','bold',...
        'String', 'Set Stimulus Parameters');
    F.tot_ht = F.tot_ht + F.ind_ht;
    
    %First Stimulus types: popupmenu
    uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht - F.tot_ht 60 F.ind_ht],...
        'ForegroundColor','blue',...
        'FontWeight','bold',...
        'String', '1st Stim:');
    H.Twosrc_stim_type = uicontrol('Parent',H.Two_sourcefig,...
        'Style','popup',...
        'Units','pixels',...
        'Position',[60 F.Two_source_ht-F.tot_ht 120 F.ind_ht],...
        'BackgroundColor','white',...
        'String',...
        'Tone|Gammatone|Tone Stack|GammaTone Stack|BandPassed Noise|NarrowBand Noise|NarrowBand Noise Stack|BroadBand Noise|File|None',...
        'Value',get(H.stim_type,'Value'),...
        'Callback','setinfo_Two_source');
    
    % only one will be displayed, either
    H.Twosrc_stim_filenamepb = uicontrol('Parent',H.Two_sourcefig,...
        'Style','pushbutton',...
        'Units','pixels',...
        'Position',[180 F.Two_source_ht-F.tot_ht 25 F.ind_ht],...
        'BackgroundColor','red',...
        'ForegroundColor','blue',...
        'String', 'FN',...
        'Callback','setinfo_Two_source');
    H.Twosrc_stim_filename = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[205 F.Two_source_ht-F.tot_ht 105 F.ind_ht],...
        'ForegroundColor','blue',...
        'String', FN.stim);
    % or
    H.Twosrc_freq_txt = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[180 F.Two_source_ht-F.tot_ht 90 F.ind_ht],...
        'Visible','off',...
        'String', 'Frequency');
    H.Twosrc_freq = uicontrol('Parent',H.Two_sourcefig,...
        'Style','edit',...
        'Units','pixels',...
        'Visible','off',...
        'Position',[270 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.freq(1)),...
        'Callback','setinfo_Two_source');
    
    uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[340 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'String', 'factor');
    H.Twosrc_factor = uicontrol('Parent',H.Two_sourcefig,...
        'Style','edit',...
        'Units','pixels',...
        'Position',[370 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'String', num2str(XStimParams.factor(1)),...
        'Callback','setinfo_Two_source');
    
    % modulate 1st sound
    F.tot_ht = F.tot_ht + F.ind_ht;
    switch XStimParams.mod_type
        case 'Tone'
            mod_num = 1;
        case 'File'
            mod_num = 2;
        case 'LP Noise'
            mod_num = 3;
        case 'None'
            mod_num = 4;
        otherwise
            mod_num = 1;
    end
    uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'FontWeight','bold',...
        'Position',[0 F.Two_source_ht - F.tot_ht 80 F.ind_ht],...
        'String', '   Modulation:');
    H.Twosrc_mod_type = uicontrol('Parent',H.Two_sourcefig,...
        'Style','popup',...
        'Units','pixels',...
        'Position',[80 F.Two_source_ht-F.tot_ht 100 F.ind_ht],...
        'BackgroundColor','White',...
        'String',...
        'Tone|LP Noise|File|None',...
        'Value',mod_num,...
        'Callback','setinfo_Two_source');
    H.Twosrc_mod_txt = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'HorizontalAlignment','right',...
        'Position',[180 F.Two_source_ht-F.tot_ht 50 F.ind_ht],...
        'String', 'Mod Freq: ');
    H.Twosrc_mod_freq = uicontrol('Parent',H.Two_sourcefig,...
        'Style','edit',...
        'Units','pixels',...
        'Position',[230 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.mod_freq(1)),...
        'Callback','setinfo_Two_source');
    
    H.Twosrc_mod_txtA = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[270 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'String', 'depth');
    H.Twosrc_mod_depth = uicontrol('Parent',H.Two_sourcefig,...
        'Style','edit',...
        'Units','pixels',...
        'Position',[300 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.mod_depth(1)),...
        'Callback','setinfo_Two_source');
    H.Twosrc_mod_txtB = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[340 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'String', 'phase');
    H.Twosrc_mod_phase = uicontrol('Parent',H.Two_sourcefig,...
        'Style','edit',...
        'Units','pixels',...
        'Position',[370 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'String', num2str(XStimParams.mod_phase(1)),...
        'Callback','setinfo_Two_source');
    
    H.Twosrc_mod_pb = uicontrol('Parent',H.Two_sourcefig,...
        'Style','pushbutton',...
        'Units','pixels',...
        'Position',[200 F.Two_source_ht-F.tot_ht 25 F.ind_ht],...
        'BackgroundColor','red',...
        'ForegroundColor','blue',...
        'String', 'FN',...
        'Visible','off',...
        'Callback','setinfo_Two_source');
    
    %Second Stimulus types: popupmenu
    F.tot_ht = F.tot_ht + F.ind_ht;
    uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht - F.tot_ht 60 F.ind_ht],...
        'ForegroundColor','blue',...
        'FontWeight','bold',...
        'String', '2nd Stim:');
    H.Twosrc_stim_type2 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','popup',...
        'Units','pixels',...
        'Position',[60 F.Two_source_ht-F.tot_ht 120 F.ind_ht],...
        'BackgroundColor','white',...
        'String',...
        'Tone|Gammatone|Tone Stack|GammaTone Stack|BandPassed Noise|NarrowBand Noise|NarrowBand Noise Stack|BroadBand Noise|File|None',...
        'Value',get(H.stim_type2,'Value'),...
        'Callback','setinfo_Two_source');
    
    % only one will be visible, either:
    H.Twosrc_stim_filename2pb = uicontrol('Parent',H.Two_sourcefig,...
        'Style','pushbutton',...
        'Units','pixels',...
        'Position',[180 F.Two_source_ht-F.tot_ht 25 F.ind_ht],...
        'BackgroundColor','red',...
        'ForegroundColor','blue',...
        'String', 'FN',...
        'Callback','setinfo_Two_source');
    H.Twosrc_stim_filename2 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[205 F.Two_source_ht-F.tot_ht 105 F.ind_ht],...
        'ForegroundColor','blue',...
        'String', FN.stim2);
    % or
    H.Twosrc_freq2_txt = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[180 F.Two_source_ht-F.tot_ht 90 F.ind_ht],...
        'Visible','off',...
        'String', 'Frequency');
    H.Twosrc_freq2 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','edit',...
        'Units','pixels',...
        'Visible','off',...
        'Position',[270 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.freq(2)),...
        'Callback','setinfo_Two_source');
    
    uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[340 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'String', 'factor');
    H.Twosrc_factor2 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','edit',...
        'Units','pixels',...
        'Position',[370 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'String', num2str(XStimParams.factor(2)),...
        'Callback','setinfo_Two_source');
    
    % modulate 2nd sound
    F.tot_ht = F.tot_ht + F.ind_ht;
    switch XStimParams.mod_type2
        case 'Tone'
            mod_num = 1;
        case 'File'
            mod_num = 2;
        case 'LP Noise'
            mod_num = 3;
        case 'None'
            mod_num = 4;
        otherwise
            mod_num = 1;
    end
    uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'FontWeight','bold',...
        'Position',[0 F.Two_source_ht - F.tot_ht 80 F.ind_ht],...
        'String', '  Modulation:');
    H.Twosrc_mod_type2 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','popup',...
        'Units','pixels',...
        'Position',[80 F.Two_source_ht-F.tot_ht 100 F.ind_ht],...
        'BackgroundColor','White',...
        'String',...
        'Tone|LP Noise|File|None',...
        'Value',mod_num,...
        'Callback','setinfo_Two_source');
    H.Twosrc_mod_txt2 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'HorizontalAlignment','right',...
        'Position',[180 F.Two_source_ht-F.tot_ht 50 F.ind_ht],...
        'String', 'Mod Freq:');
    H.Twosrc_mod_freq2 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','edit',...
        'Units','pixels',...
        'Position',[230 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.mod_freq(2)),...
        'Callback','setinfo_Two_source');
    H.Twosrc_mod_txtA2 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[270 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'String', 'depth');
    H.Twosrc_mod_depth2 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','edit',...
        'Units','pixels',...
        'Position',[300 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.mod_depth(2)),...
        'Callback','setinfo_Two_source');
    H.Twosrc_mod_txtB2 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[340 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'String', 'phase');
    H.Twosrc_mod_phase2 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','edit',...
        'Units','pixels',...
        'Position',[370 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'String', num2str(XStimParams.mod_phase(2)),...
        'Callback','setinfo_Two_source');
    
    H.Twosrc_mod_pb2 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','pushbutton',...
        'Units','pixels',...
        'Position',[200 F.Two_source_ht-F.tot_ht 25 F.ind_ht],...
        'BackgroundColor','red',...
        'ForegroundColor','blue',...
        'String', 'FN',...
        'Visible','off',...
        'Callback','setinfo_Two_source');
    
    %Third Stimulus types: popupmenu
    F.tot_ht = F.tot_ht + F.ind_ht;
    uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht - F.tot_ht 60 F.ind_ht],...
        'ForegroundColor','blue',...
        'FontWeight','bold',...
        'String', '3rd Stim:');
    H.Twosrc_stim_type3 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','popup',...
        'Units','pixels',...
        'Position',[60 F.Two_source_ht-F.tot_ht 120 F.ind_ht],...
        'BackgroundColor','white',...
        'String',...
        'Tone|Gammatone|Tone Stack|GammaTone Stack|BandPassed Noise|NarrowBand Noise|NarrowBand Noise Stack|BroadBand Noise|File|None',...
        'Value',get(H.stim_type3,'Value'),...
        'Callback','setinfo_Two_source');
    
    % only one will be displayed, either
    H.Twosrc_stim_filenamepb3 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','pushbutton',...
        'Units','pixels',...
        'Position',[180 F.Two_source_ht-F.tot_ht 25 F.ind_ht],...
        'BackgroundColor','red',...
        'ForegroundColor','blue',...
        'String', 'FN',...
        'Callback','setinfo_Two_source');
    H.Twosrc_stim_filename3 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[205 F.Two_source_ht-F.tot_ht 105 F.ind_ht],...
        'ForegroundColor','blue',...
        'String', FN.stim3);
    % or
    H.Twosrc_freq_txt3 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[180 F.Two_source_ht-F.tot_ht 90 F.ind_ht],...
        'Visible','off',...
        'String', 'Frequency');
    H.Twosrc_freq3 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','edit',...
        'Units','pixels',...
        'Visible','off',...
        'Position',[270 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.freq(3)),...
        'Callback','setinfo_Two_source');
    
    uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[340 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'String', 'factor');
    H.Twosrc_factor3 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','edit',...
        'Units','pixels',...
        'Position',[370 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'String', num2str(XStimParams.factor(3)),...
        'Callback','setinfo_Two_source');    
    
    % modulate 3rd sound
    F.tot_ht = F.tot_ht + F.ind_ht;
    switch XStimParams.mod_type
        case 'Tone'
            mod_num = 1;
        case 'File'
            mod_num = 2;
        case 'LP Noise'
            mod_num = 3;
        case 'None'
            mod_num = 4;
        otherwise
            mod_num = 1;
    end 
    uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'FontWeight','bold',...
        'Position',[0 F.Two_source_ht - F.tot_ht 80 F.ind_ht],...
        'String', '  Modulation:');
    H.Twosrc_mod_type3 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','popup',...
        'Units','pixels',...
        'Position',[80 F.Two_source_ht-F.tot_ht 100 F.ind_ht],...
        'BackgroundColor','White',...
        'String',...
        'Tone|LP Noise|File|None',...
        'Value',mod_num,...
        'Callback','setinfo_Two_source');
    H.Twosrc_mod_txt3 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'HorizontalAlignment','right',...
        'Position',[180 F.Two_source_ht-F.tot_ht 50 F.ind_ht],...
        'String', 'Mod Freq: ');
    H.Twosrc_mod_freq3 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','edit',...
        'Units','pixels',...
        'Position',[230 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.mod_freq(3)),...
        'Callback','setinfo_Two_source');
    
    H.Twosrc_mod_txtA3 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[270 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'String', 'depth');
    H.Twosrc_mod_depth3 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','edit',...
        'Units','pixels',...
        'Position',[300 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.mod_depth(3)),...
        'Callback','setinfo_Two_source');
    H.Twosrc_mod_txtB3 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','text',...
        'Units','pixels',...
        'Position',[340 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'String', 'phase');
    H.Twosrc_mod_phase3 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','edit',...
        'Units','pixels',...
        'Position',[370 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'String', num2str(XStimParams.mod_phase(3)),...
        'Callback','setinfo_Two_source');
    
    H.Twosrc_mod_pb3 = uicontrol('Parent',H.Two_sourcefig,...
        'Style','pushbutton',...
        'Units','pixels',...
        'Position',[200 F.Two_source_ht-F.tot_ht 25 F.ind_ht],...
        'BackgroundColor','red',...
        'ForegroundColor','blue',...
        'String', 'FN',...
        'Visible','off',...
        'Callback','setinfo_Two_source');
    
    F.tot_ht = F.tot_ht + F.ind_ht;
    H.Twosrc_DynamicRangepb = uicontrol('Style','togglebutton',...
        'Units','pixels',...
        'Position',[180 F.Two_source_ht-F.tot_ht F.Two_source_wd-180 F.ind_ht],...
        'BackgroundColor','yellow',...
        'ForegroundColor','blue',...
        'String', 'push for High Dynamic Range',...
        'Callback','setinfo_Two_source');
    
    H.Twosrc_DynamicRangeTxt = uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht 180 F.ind_ht],...
        'BackgroundColor',[.8 .8 .8],...
        'String', 'SPL now 20dB below normal');
   
    % offset source2 from source1
    F.tot_ht = F.tot_ht + F.ind_ht;
    F.tot_ht = F.tot_ht + F.ind_ht;
     uicontrol('Style','text',...
        'BackgroundColor',[.7 .7 .7],...
        'Units','pixels',...
        'Position',[250 F.Two_source_ht-F.tot_ht 120 F.ind_ht],...
        'String', 'loc1 = loc + offset/2');
    
   F.tot_ht = F.tot_ht + F.ind_ht;
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht 240 F.ind_ht],...
        'String', 'Offsets (twice distance from array-center) ');
    
    uicontrol('Style','text',...
        'BackgroundColor',[.7 .7 .7],...
        'Units','pixels',...
        'Position',[250 F.Two_source_ht-F.tot_ht 120 F.ind_ht],...
        'String', 'loc2 = loc - offset/2');

    F.tot_ht = F.tot_ht + F.ind_ht;
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht 80 F.ind_ht],...
        'String', 'Src 1,2 Elev');
    H.offset_el = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[80 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.offset_el(2)),...
        'Callback','setinfo_Two_source');
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[125 F.Two_source_ht-F.tot_ht 75 F.ind_ht],...
        'String', 'Src 1,2 Azim');
    H.offset_az = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[200 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.offset_az(2)),...
        'Callback','setinfo_Two_source');
    
    
        
uicontrol('Style','text',...
        'BackgroundColor',[.7 .7 .7],...
        'Units','pixels',...
        'Position',[250 F.Two_source_ht-F.tot_ht 120 F.ind_ht],...
        'String', '  loc3 = loc + offset3/2');
    

    
    
    % offset 3rd source from first source
    F.tot_ht = F.tot_ht + F.ind_ht;
    H.Stims_txt1 = uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht 80 F.ind_ht],...
        'String', 'Src 3 Elev');
    H.offset_el2 = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[80 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.offset_el(3)),...
        'Callback','setinfo_Two_source');
    H.Stims_txt2 = uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[125 F.Two_source_ht-F.tot_ht 75 F.ind_ht],...
        'String', 'Src 3 Azim');
    H.offset_az2 = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[200 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.offset_az(3)),...
        'Callback','setinfo_Two_source');
    
    % use third sound??
    H.Use3Stims_txt = uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[250 F.Two_source_ht-F.tot_ht 100 F.ind_ht],...
        'String', 'Use 3rd source?');
    H.Use3Stims = uicontrol('Parent',H.Two_sourcefig,...
        'Style','checkbox',...
        'Units','pixels',...
        'Position',[350 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'Value',(XStimParams.ThreeStims),...
        'Callback','setInfo_Two_source');        

    % bandwidths
    F.tot_ht = F.tot_ht + F.ind_ht;
    F.tot_ht = F.tot_ht + F.ind_ht;
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht 110 F.ind_ht],...
        'String', 'Bandwidth(oct)   src1:');
    H.bandwidth1 = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[112 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.bandwidth(1)),...
        'Callback','setinfo_Two_source');
    
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[180 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', 'src2:');
    H.bandwidth2 = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[222 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.bandwidth(2)),...
        'Callback','setinfo_Two_source');
    
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[280 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', 'src3:');
    H.bandwidth3 = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[320 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.bandwidth(3)),...
        'Callback','setinfo_Two_source');
    
    % bandlimits
    F.tot_ht = F.tot_ht + F.ind_ht;
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht 50 F.ind_ht],...
        'String', 'limits  s1:');
    H.bandlimits1 = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[52 F.Two_source_ht-F.tot_ht 50 F.ind_ht],...
        'String', num2str(XStimParams.bandlimits(1)),...
        'Callback','setinfo_Two_source');
    H.bandlimits2 = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[102 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.bandlimits(2)),...
        'Callback','setinfo_Two_source');
    
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[144 F.Two_source_ht-F.tot_ht 24 F.ind_ht],...
        'String', 's2:');
    H.bandlimits3 = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[168 F.Two_source_ht-F.tot_ht 50 F.ind_ht],...
        'String', num2str(XStimParams.bandlimits(3)),...
        'Callback','setinfo_Two_source');
    H.bandlimits4 = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[218 F.Two_source_ht-F.tot_ht 50 F.ind_ht],...
        'String', num2str(XStimParams.bandlimits(4)),...
        'Callback','setinfo_Two_source');
    
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[270 F.Two_source_ht-F.tot_ht 24 F.ind_ht],...
        'String', 's3:');
    H.bandlimits5 = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[294 F.Two_source_ht-F.tot_ht 50 F.ind_ht],...
        'String', num2str(XStimParams.bandlimits(5)),...
        'Callback','setinfo_Two_source');
    H.bandlimits6 = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[344 F.Two_source_ht-F.tot_ht 50 F.ind_ht],...
        'String', num2str(XStimParams.bandlimits(6)),...
        'Callback','setinfo_Two_source');
    
    % constant ITD ?
    F.tot_ht = F.tot_ht + F.ind_ht;
    F.tot_ht = F.tot_ht + F.ind_ht;
    H.ITDtxt = uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht 90 F.ind_ht],...
        'BackgroundColor',[.8 .8 .8],...
        'String', 'ITD constant      ');
    
    % ITD
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[97 F.Two_source_ht-F.tot_ht 60 F.ind_ht],...
        'String', 'ITD(us):');
    H.ITD = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[157 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'String', num2str(XStimParams.curr_ITD),...
        'Callback', 'setinfo_Two_source');
    
    % constant ILD ?
    F.tot_ht = F.tot_ht + F.ind_ht;
    H.ILDtxt = uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht 90 F.ind_ht],...
        'BackgroundColor',[.8 .8 .8],...
        'String', 'ILD constant      ');
    
    % 1st source ILD
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[97 F.Two_source_ht-F.tot_ht 20 F.ind_ht],...
        'String', 'el');
    H.el = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[117 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'String', num2str(XStimParams.el),...
        'Callback', 'setinfo_Two_source');
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[147 F.Two_source_ht-F.tot_ht 20 F.ind_ht],...
        'String', 'az');
    H.az = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[167 F.Two_source_ht-F.tot_ht 30 F.ind_ht],...
        'String', num2str(XStimParams.az),...
        'Callback', 'setinfo_Two_source');
           
    %ABL
    F.tot_ht = F.tot_ht + F.ind_ht;
    F.tot_ht = F.tot_ht + F.ind_ht;
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht 80 F.ind_ht],...
        'String', 'Atten (dB):');
    H.ABL = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[82 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.curr_ABL),...
        'Callback', 'setinfo_Two_source');
    
    %ISI
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[140 F.Two_source_ht-F.tot_ht 60 F.ind_ht],...
        'String', 'ISI (ms):');
    H.ISI = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[202 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.test_ISI),...
        'Callback', 'setinfo_Two_source');
    
    %DUR
    F.tot_ht = F.tot_ht + F.ind_ht;
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht 80 F.ind_ht],...
        'String', 'Stim Dur (ms):');
    H.DUR = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[82 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.curr_stimdur),...
        'Callback','setinfo_Two_source');
    
    %NumReps
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[140 F.Two_source_ht-F.tot_ht 60 F.ind_ht],...
        'String', '# Reps:');
    H.numreps = uicontrol('Style','edit',...
        'Units','pixels',...
        'Position',[202 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', num2str(XStimParams.numreps),...
        'Callback','setinfo_Two_source');
    
    % run control
    F.tot_ht = F.tot_ht + F.ind_ht + F.pos_incr;
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht F.Two_source_wd F.ind_ht],...
        'BackgroundColor','blue',...
        'ForegroundColor','white',...
        'FontWeight','bold',...
        'String', 'Run Control');
    
    %Engage TEST
    F.tot_ht = F.tot_ht + F.ind_ht;
    H.engageTwo_source = uicontrol('Style','pushbutton',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht 80 F.ind_ht],...
        'BackgroundColor','green',...
        'ForegroundColor','white',...
        'FontWeight','bold',...
        'String', 'ENGAGE',...
        'Callback','Engage_Two_source');
    
    %Record Data?
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[85 F.Two_source_ht-F.tot_ht 100 F.ind_ht],...
        'String', 'Record Data?');
    H.recorddata = uicontrol('Style','checkbox',...
        'Units','pixels',...
        'Position',[185 F.Two_source_ht-F.tot_ht 20 F.ind_ht],...
        'Callback', 'setinfo_Two_source');
    F.tot_ht = F.tot_ht + F.ind_ht;
    H.recorddata_FN = uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[85 F.Two_source_ht-F.tot_ht 100 F.ind_ht],...
        'String', FN.data);
    
    % increment test number
    H.inc_testnum = uicontrol('Style','pushbutton',...
        'Units','pixels',...
        'Position',[250 F.Two_source_ht-F.tot_ht 140 F.ind_ht],...
        'BackgroundColor','yellow',...
        'ForegroundColor','blue',...
        'FontWeight','bold',...
        'String', 'INCREMENT testnum',...
        'Callback','setInfo_Two_source');
    
    %Pause TEST
    F.ind_ht = 20; 
    F.tot_ht = F.tot_ht + F.ind_ht + F.pos_incr;
    H.pauseTwo_source = uicontrol('Style','togglebutton',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht 80 F.ind_ht],...
        'BackgroundColor','blue',...
        'ForegroundColor','white',...
        'FontWeight','bold',...
        'String', 'Pause');
   
     %plotVstrength
    H.plotVstr = uicontrol('Style','togglebutton',...
        'Units','pixels',...
        'Position',[100 F.Two_source_ht-F.tot_ht 80 F.ind_ht],...
        'BackgroundColor','cyan',...
        'ForegroundColor','black',...
        'FontWeight','bold',...
        'String', 'Plot Vstr',...
        'Callback', 'setInfo_Two_source');

    %Reset
    H.resetTwo_source = uicontrol('Style','togglebutton',...
        'Units','pixels',...
        'Position',[200 F.Two_source_ht-F.tot_ht 80 F.ind_ht],...
        'BackgroundColor','cyan',...
        'ForegroundColor','black',...
        'FontWeight','bold',...
        'String', 'Reset',...
        'Callback','Reset_Two_source');
    
    %Exit
    H.exitTwo_source = uicontrol('Style','togglebutton',...
        'Units','pixels',...
        'Position',[F.Two_source_wd-100 F.Two_source_ht-F.tot_ht 80 F.ind_ht],...
        'BackgroundColor','red',...
        'ForegroundColor','yellow',...
        'FontWeight','bold',...
        'String', 'EXIT',...
        'Callback','Exit_Two_source');
    
    %Build/Play Trials
    F.ind_ht = 20; 
    F.tot_ht = F.tot_ht + F.ind_ht + F.pos_incr;
    H.status = uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht F.Two_source_wd F.ind_ht],...
        'BackgroundColor','blue',...
        'ForegroundColor','white',...
        'FontWeight','bold',...
        'String', 'Status: setting parameters');
    
    %Remaining Reps and Trials
    F.tot_ht = F.tot_ht + F.ind_ht;
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[0 F.Two_source_ht-F.tot_ht 120 F.ind_ht],...
        'String', 'Remaining reps:');
    H.remreps = uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[122 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', get(H.numreps,'String'));
    uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[164 F.Two_source_ht-F.tot_ht 80 F.ind_ht],...
        'String', 'Trials:');
    H.remtrials = uicontrol('Style','text',...
        'Units','pixels',...
        'Position',[246 F.Two_source_ht-F.tot_ht 40 F.ind_ht],...
        'String', '');
    
end %end GUI specification

stimuli_dir = FN.temp_stim_path;

% Initialize Application and get AP2 and XBUS locks
if(S232('S2init', 0, 'INIT_PRIMARY', 20000) == 0)
    disp('Cannot initialize a secondary process')
    return;
end

if(S232('APlock', 100, 0) == 0)
    disp('Cannot acquire lock on AP2 Card')
    s232('S2close');
    return;
end

if(S232('XBlock', 100, 0) == 0)
    disp('Cannot acquire lock on X Bus')
    s232('APunlock', 0);
    s232('S2close');
    return;
end

SetInfo_Two_source;