...
elseif(XStimParams.itdalone_flag == 1)
     if isempty(FN.itdalone)        % FN not yet picked
        [FN.itdalone,FN.ITA_path] = uigetfile('*.*','Select ITD-alone HRTF File');
        if(FN.ITA_path ~= 0)
...
if ~XStimParams.alter_setbands
    if XStimParams.alterMag
        if isempty(find(XStimParams.Mag_octlistBIN ~=0))
            for i = 1:8
                set(H.Mag_octlist(i),'backgroundcolor','m');
            end
            disp('Please check frequencies');
            pause
            for i = 1:8
                set(H.Mag_octlist(i),'backgroundcolor',[.8 .8 .8]);
            end
            return
        end
    end
    if XStimParams.alterPhase
        if isempty(find(XStimParams.Phase_octlistBIN ~=0))
            for i = 1:8
                set(H.Phase_octlist(i),'backgroundcolor','m');
            end
            disp('Please check frequencies');
            pause
            for i = 1:8
                set(H.Phase_octlist(i),'backgroundcolor',[.8 .8 .8]);
            end
            return
        end
    end
    ...
    while (exist1('H.altIRfig') & (trialnum <= numtrials))
        ...
        if(XStimParams.space_flag == 1)                 % FC
            ...
            if XStimParams.alter_left
                ...
                else            % use 1/3 octave bands
                    if XStimParams.alterMag
                        hrtf_leftX = altmagIR4 (hrtf_left, find(XStimParams.Mag_octlistBIN ~=0), XStimParams.alterMag_dB);
                    end
                    if XStimParams.alterPhase
                        hrtf_leftX = altphaseIR4 (hrtf_left, find(XStimParams.Phase_octlistBIN ~=0), XStimParams.alterPhase_rad);
                    end
                end    
            end   
            % alter phase of IRs     
            if XStimParams.alter_right
                if XStimParams.alter_setbands
                    ...
                    if XStimParams.alterMag
                        hrtf_rightX = altmagIR4 (hrtf_right, find(XStimParams.Mag_octlistBIN ~=0), XStimParams.alterMag_dB);
                    end
                    if XStimParams.alterPhase
                        hrtf_rightX = altphaseIR4 (hrtf_right, find(XStimParams.Phase_octlistBIN ~=0), XStimParams.alterPhase_rad);
                    end
                end
            end   
