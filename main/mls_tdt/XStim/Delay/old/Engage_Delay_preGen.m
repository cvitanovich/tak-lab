...
% Questions: How do I save a (100+50 ms) sound so that it can be loaded with get_stim?
...
if(XStimParams.randOnsetPerms)
    ...
    %disp(DelayTimes);
    MaxDelay = max1(abs(DelayTimes));
    MaxDelay_pnts=round(TDT.Fs*(MaxDelay/1000));
    %disp(MaxDelay_pnts);
    ...
    MaxLag = abs(XStimParams.lag_seg_ext(1));
end
...
if (get(H.stim_type,'Value') == 9) & isempty(FN.stim)
    [FN.stim,FN.stim_path] = uigetfile([FN.stim_path '*'],'Select snd File');
end
...
MaxLagPnts = abs(MaxLag*round(TDT.Fs/1000));
%disp(MaxDelay_pnts);
...
if XStimParams.ildalone_flag 					%ILDAlone Test
    S232('allot16',BUF.L1,(silence_len2 + silence_len + DUR*round(TDT.Fs/1000)) + TDT.itdfiltlen + TDT.ephonefiltlen + TDT.hrtffiltlen + abs(MaxDelay_pnts) + MaxLagPnts);
    S232('allot16',BUF.R1,(silence_len2 + silence_len + DUR*round(TDT.Fs/1000)) + TDT.itdfiltlen + TDT.ephonefiltlen + TDT.hrtffiltlen + abs(MaxDelay_pnts) + MaxLagPnts);
    S232('allot16',BUF.L2,(silence_len2 + silence_len + DUR*round(TDT.Fs/1000)) + TDT.itdfiltlen + TDT.ephonefiltlen + TDT.hrtffiltlen + abs(MaxDelay_pnts) + MaxLagPnts);
    S232('allot16',BUF.R2,(silence_len2 + silence_len + DUR*round(TDT.Fs/1000)) + TDT.itdfiltlen + TDT.ephonefiltlen + TDT.hrtffiltlen + abs(MaxDelay_pnts) + MaxLagPnts);
elseif XStimParams.itdalone_flag 				%ITDAlone Test
    S232('allot16',BUF.L1,(silence_len2 + silence_len + DUR*round(TDT.Fs/1000)) + TDT.ephonefiltlen + TDT.hrtffiltlen*2 + abs(MaxDelay_pnts) + MaxLagPnts);
    S232('allot16',BUF.R1,(silence_len2 + silence_len + DUR*round(TDT.Fs/1000)) + TDT.ephonefiltlen + TDT.hrtffiltlen*2 + abs(MaxDelay_pnts) + MaxLagPnts);
    S232('allot16',BUF.L2,(silence_len2 + silence_len + DUR*round(TDT.Fs/1000)) + TDT.ephonefiltlen + TDT.hrtffiltlen*2 + abs(MaxDelay_pnts) + MaxLagPnts);
    S232('allot16',BUF.R2,(silence_len2 + silence_len + DUR*round(TDT.Fs/1000)) + TDT.ephonefiltlen + TDT.hrtffiltlen*2 + abs(MaxDelay_pnts));
elseif XStimParams.space_flag | XStimParams.ABLalone_flag	    %fully-cued Test
    S232('allot16',BUF.L1,(silence_len2 + silence_len + DUR*round(TDT.Fs/1000)) + TDT.ephonefiltlen + TDT.hrtffiltlen + abs(MaxDelay_pnts) + MaxLagPnts);
    S232('allot16',BUF.R1,(silence_len2 + silence_len + DUR*round(TDT.Fs/1000)) + TDT.ephonefiltlen + TDT.hrtffiltlen + abs(MaxDelay_pnts) + MaxLagPnts);
    S232('allot16',BUF.L2,(silence_len2 + silence_len + DUR*round(TDT.Fs/1000)) + TDT.ephonefiltlen + TDT.hrtffiltlen + abs(MaxDelay_pnts) + MaxLagPnts);
    S232('allot16',BUF.R2,(silence_len2 + silence_len + DUR*round(TDT.Fs/1000)) + TDT.ephonefiltlen + TDT.hrtffiltlen + abs(MaxDelay_pnts) + MaxLagPnts);
end   

S232('PD1clear',1);
S232('PD1srate', 1, 1e6/TDT.Fs);
if XStimParams.ildalone_flag 					%ILDAlone Test
    S232('PD1npts',1,(silence_len2 + silence_len + DUR*(round(TDT.Fs/1000))) + TDT.itdfiltlen  + TDT.ephonefiltlen + TDT.hrtffiltlen + abs(MaxDelay_pnts) + MaxLagPnts);
elseif XStimParams.itdalone_flag 				%ITDAlone Test
    S232('PD1npts',1,(silence_len2 + silence_len + DUR*(round(TDT.Fs/1000)))  + TDT.ephonefiltlen + (2 * TDT.hrtffiltlen) + abs(MaxDelay_pnts) + MaxLagPnts);
elseif XStimParams.space_flag |  XStimParams.ABLalone_flag			% fully-cued or ABL-alone Test
    S232('PD1npts',1,(silence_len2 + silence_len + DUR*(round(TDT.Fs/1000))) + TDT.ephonefiltlen + TDT.hrtffiltlen + abs(MaxDelay_pnts) + MaxLagPnts);
end
...
while (exist1('H.Delayfig') & (trialnum <= numtrials))
   ...
   %Make the first stimulus
    ...
    if(~uncorrel)
     ...
     % modulate stim1 % adding 50 ms in get_stim !!!
    if ~strcmp(XStimParams.mod_type,'None')
        Envelope = make_env(DUR+50, XStimParams.mod_type, XStimParams.mod_depth(1), XStimParams.mod_freq(1), XStimParams.mod_phase(1));
    ...
    % delete (50-LagExtSegsSeq(trialnum)) of stim since I added 50 ms in get_stim
    ...
    SegDel_pnts=0;
    if(DelayModsSeq(trialnum)==4 & Delay_pnts>0)
        SegDel_pnts=round(TDT.Fs*(DelayTimesSeq(trialnum))/1000);
    ...
    % will buffer to MaxDelay_Pnts
     if(XStimParams.randOnsetPerms)
        ...
        if(Delay_pnts > 0)
            MaxDelayBufferPnts = MaxDelay_pnts - Delay_pnts + MaxLagPnts;
        else
            MaxDelayBufferPnts = MaxDelay_pnts + Delay_pnts + MaxLagPnts;
        end
          %MaxDelayBufferPnts = MaxDelay_pnts + MaxLagPnts;
     end
    ...
    Del_pnts = ExtDel_pnts + SegDel_pnts ;
...
%     disp(Delay_pnts);
    ...
    if(Delay_pnts<0 & SilentDelay)
        ...
        source1_L_temp(-Delay_pnts:origPnts-Del_pnts) = source1_L;  % neg delay
        source1_L = source1_L_temp;
        
        source1_R_temp=zeros(1,origPnts);
        source1_R_temp(-Delay_pnts:origPnts-Del_pnts) = source1_R;  % neg delay
        source1_R = source1_R_temp;
    else
        source1_L_temp=zeros(1,origPnts);
        source1_L_temp(1:origPnts-Del_pnts) = source1_L;  % neg delay
        source1_L = source1_L_temp;
        
        source1_R_temp=zeros(1,origPnts);
        source1_R_temp(1:origPnts-Del_pnts) = source1_R;  % neg delay
        source1_R = source1_R_temp;
    end
 ...
 %   disp(size(source1_L,2));
   ...
   if(uncorrel) % new stim 2
    ...
    % modulate stim2 % adding 50 ms in get_stim !!!
    if ~strcmp(XStimParams.mod_type,'None')
        Envelope = make_env(DUR+50, XStimParams.mod_type, XStimParams.mod_depth(1), XStimParams.mod_freq(1), XStimParams.mod_phase(1));
    ...
    % delete last 50 ms of stim since I added 50 ms in get_stim
    ExtDel_pnts=round(TDT.Fs*(50)/1000); % added 50 ms in get_stim
    source2_L(size(source2_L,2)-ExtDel_pnts : size(source2_L,2))=[];
    source2_R(size(source2_R,2)-ExtDel_pnts : size(source2_R,2))=[];
    ExtDel_pnts=ExtDel_pnts+1;
    ...
    if(DelayModsSeq(trialnum)==4 & Delay_pnts<0)
        SegDel_pnts=round(TDT.Fs*(-DelayTimesSeq(trialnum))/1000);
        source2_L(size(source2_L,2)-SegDel_pnts : size(source2_L,2))=[];
        source2_R(size(source2_R,2)-SegDel_pnts : size(source2_R,2))=[];
        SegDel_pnts=SegDel_pnts+1;
    end
...
%      disp(Delay_pnts);
   ...
   if(Delay_pnts>0 & SilentDelay)
        ...
        source2_L_temp(Delay_pnts:origPnts-Del_pnts) = source2_L;
        source2_L = source2_L_temp;
        
        source2_R_temp=zeros(1,origPnts);
        source2_R_temp(Delay_pnts:origPnts-Del_pnts) = source2_R;
        source2_R = source2_R_temp;
    else
        % replace deleted points with zeros
        source2_L_temp=zeros(1,origPnts);
        source2_L_temp(1:origPnts-Del_pnts) = source2_L;
        source2_L = source2_L_temp;
        
        source2_R_temp=zeros(1,origPnts);
        source2_R_temp(1:origPnts-Del_pnts) = source2_R;
        source2_R = source2_R_temp;
    end  
...
%     disp(size(source2_L,2));
    ...
    if(Delay_pnts>0) % delay 1st sound
        ...
        if(Delay_pnts<0) % delay 2nd sound
            ...
            source1_R_temp=zeros(1,size(source1_R,2)-Delay_pnts);
            source1_R_temp(1:size(source1_R,2))=source1_R;
            source1_R = source1_R_temp;
    ...
    if(plotAllStim | trialnum==plotStimTrial)
        ...
        hold on;
        %subplot(2,1,1);
        Stim.R = plot(timeaxis, source1_R + 0.4,...
            'Tag','Env_ellipses',...
            'Marker', 'none',...
            'LineStyle','-',...
            'Color', [1 0.5 0],...
        ...
        Stim.L = plot(timeaxis, source1_L + 0.4,...
            ...
            'LineWidth', 0.5);
        %subplot(2,1,2);
        Stim.R = plot(timeaxis, source2_R - 0.4,...
            'Tag','Env_ellipses',...
            'Marker', 'none',...
            'LineStyle','-',...
            'Color', [0 0.5 1],...
            'LineWidth', 0.5);
        Stim.L = plot(timeaxis, source2_L - 0.4,...
            'Tag','Env_ellipses',...
            'Marker', 'none',...
            'LineStyle','-',...
            'Color', [0 0 1],...
...
while (exist1('H.Delayfig') & (repnum <= XStimParams.numreps))
    ...
    if(1)
        ...
        if(findobj('Tag','Raster'))
            ...
            ylabel('trial'); xlabel('time (ms)'); title(['Rep# ' num2str(repnum)]);
            ylim([0 numtrials+1]);
            xmax=XStimParams.silence_lead +DUR +70 +MaxLag;
