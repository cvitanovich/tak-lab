...
fig_wd = position(3);
fig_ht = position(4);
set(hrastaxes,'Position',[30/fig_wd 30/fig_ht .95 .95]);
hold on;
axis off;
...
for y = 1:length(yvals)
  for x = 1:length(xvals)
     ...
     if (x==1) 
        th = text(xpos-20, ypos+(numreps+yoffset)/2, num2str(yvals(y)));
        set(th, 'HorizontalAlignment', 'center','Rotation',0);
        set(th, 'FontSize', 8, 'Color', textcolor);
     end;
     if (y == 1 & length(xvals)>1)
        th = text(xpos+(tot_dur+xoffset)/2, ypos-1, num2str(xvals(x))); 
        set(th, 'HorizontalAlignment', 'center');
        set(th, 'VerticalAlignment', 'top','Rotation',0);
        set(th, 'FontSize', 8, 'Color', textcolor);
     end;
  end;
end;
