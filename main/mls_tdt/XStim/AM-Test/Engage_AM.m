...
%*******************************************************************************
% NOTE: should use HRIRs without earphone equalization - usually called *.std
% Fully cued only.  Some processing for other tests is missing.
%*******************************************************************************
...
%%%%%%%%%%%%%%%%%%% play trial=1, rep=1, subrep=1 as a blank sound first to work-around TDT glitch
set(H.status,'String', 'Status: playing blank trial');
S232('dropall');


FNtempL = ['stimbuf_left_' num2str(hrtfdirmat(1,locind(1))) ...
        '_' num2str(hrtfdirmat(2,locind(1))) '_' num2str(1) '_' num2str(1)];
FNtempR = ['stimbuf_right_' num2str(hrtfdirmat(1,locind(1))) ...
        '_' num2str(hrtfdirmat(2,locind(1))) '_' num2str(1) '_' num2str(1)];

FNtempL = [FN.temp_stim_path FNtempL];
FNtempR = [FN.temp_stim_path FNtempR];
...
while (exist1('H.AM_fig') & (repnum <= XStimParams.numreps))
 ...
 while (exist1('H.AM_fig') & (trialnum <= numtrials))
    ...
    sprintf('on subrep %d of %d, rep %d of %d, trial %d of %d', subrepnum, numsubreps, repnum, XStimParams.numreps, trialnum, numtrials)
    
     
     %Check for pause by user
        if pause_check    return; end
        
%       instead of loading whole stimulus into BUF.L1 and BUF.L2, load
%       only first segment

        FNtempL = ['stimbuf_left_' num2str(hrtfdirmat(1,locind(randseq(trialnum)))) ...
                '_' num2str(hrtfdirmat(2,locind(randseq(trialnum)))) '_' num2str(subrepnum) '_' num2str(repnum)];
        FNtempL = [FN.temp_stim_path FNtempL];
...
%       eval(evalstr);
        FNtempR = ['stimbuf_right_' num2str(hrtfdirmat(1,locind(randseq(trialnum)))) ...
                '_' num2str(hrtfdirmat(2,locind(randseq(trialnum)))) '_' num2str(subrepnum) '_' num2str(repnum)];
        FNtempR = [FN.temp_stim_path FNtempR];
%       evalstr = ['S232(''pushdisk16''' ',' [' ''' FN.temp_stim_path fname ''' '] ');'];
%       eval(evalstr);
